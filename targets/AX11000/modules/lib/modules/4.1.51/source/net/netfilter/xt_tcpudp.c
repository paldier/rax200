...
#if 1 // 2015/07/16 Mos, Add one by one option for portforwading in different port range
	// external port start will be used in nf_nat_proto_unique_tuple()
	ct = nf_ct_get(skb, &ctinfo);
	if (ct != NULL)
	{
		ct->ext_port_start = tcpinfo->dpts[0];
	}   
#endif
	return true;
}
...
#if 1 // 2015/07/16 Mos, Add one by one option for portforwading in different port range
	// external port start will be used in nf_nat_proto_unique_tuple()
	if ((port_match(udpinfo->spts[0], udpinfo->spts[1], ntohs(uh->source), (udpinfo->invflags & XT_UDP_INV_SRCPT))
		&& port_match(udpinfo->dpts[0], udpinfo->dpts[1], ntohs(uh->dest), !!(udpinfo->invflags & XT_UDP_INV_DSTPT))))
	{
		ct = nf_ct_get(skb, &ctinfo);
		if (ct != NULL)
		{
			ct->ext_port_start = udpinfo->dpts[0];
		}   
		return true;
	}
	else
