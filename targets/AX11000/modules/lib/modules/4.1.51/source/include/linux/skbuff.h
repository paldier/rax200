 ...
 */
	...
	/* Foxconn modify start, Max Ding, 12/21/2016 R8000P's kernel is 64bit, so need reserve 8 bytes for point, otherwise DUT will crash in some cases, use fpath_cb 20-27 for pp_bridge_indev point */
//#define pp_bridge_indev(skb) (struct net_device **)(&(skb->cb[44]))
#define pp_bridge_indev(skb) (struct net_device **)(&(skb->fpath_cb[20]))
	/* Foxconn modify end, Max Ding, 12/21/2016 */

#if defined(CONFIG_BCM_KF_WL)
    union {
        __u32 wl_pktc[5]; /* can optimize zeroing of the structure below */
        struct {
