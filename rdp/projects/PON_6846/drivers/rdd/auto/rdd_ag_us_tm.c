...
{
    if(_entry >= RDD_US_TM_TM_FLOW_CNTR_TABLE_SIZE)
          return BDMF_ERR_PARM;

    RDD_TM_FLOW_CNTR_ENTRY_CNTR_ID_WRITE_G(cntr_id, RDD_US_TM_TM_FLOW_CNTR_TABLE_ADDRESS_ARR, _entry);
...
int rdd_ag_us_tm_tm_flow_cntr_table_get(uint32_t _entry, uint8_t *cntr_id)
{
    ...
    RDD_TM_FLOW_CNTR_ENTRY_CNTR_ID_READ_G(*cntr_id, RDD_US_TM_TM_FLOW_CNTR_TABLE_ADDRESS_ARR, _entry);
...
{
    RDD_BYTES_2_BITS_WRITE_G(bits, RDD_US_TM_BB_DESTINATION_TABLE_ADDRESS_ARR, 0);
...
{
    RDD_BYTES_2_BITS_READ_G(*bits, RDD_US_TM_BB_DESTINATION_TABLE_ADDRESS_ARR, 0);
...
{
    if(_entry >= RDD_FLUSH_DISPATCHER_CREDIT_TABLE_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTES_4_BITS_WRITE_G(bits, RDD_FLUSH_DISPATCHER_CREDIT_TABLE_ADDRESS_ARR, _entry);
...
int rdd_ag_us_tm_flush_dispatcher_credit_table_get(uint32_t _entry, uint32_t *bits)
{
    ...
    RDD_BYTES_4_BITS_READ_G(*bits, RDD_FLUSH_DISPATCHER_CREDIT_TABLE_ADDRESS_ARR, _entry);
...
int rdd_ag_us_tm_bbh_tx_us_fifo_bytes_threshold_set(uint16_t bits)
{
    RDD_BYTES_2_BITS_WRITE_G(bits, RDD_BBH_TX_US_FIFO_BYTES_THRESHOLD_ADDRESS_ARR, 0);

    return BDMF_ERR_OK;
}

int rdd_ag_us_tm_bbh_tx_us_fifo_bytes_threshold_get(uint16_t *bits)
{
    RDD_BYTES_2_BITS_READ_G(*bits, RDD_BBH_TX_US_FIFO_BYTES_THRESHOLD_ADDRESS_ARR, 0);
...
int rdd_ag_us_tm_bbh_tx_fifo_size_set(uint8_t bits)
{
    RDD_BYTE_1_BITS_WRITE_G(bits, RDD_BBH_TX_FIFO_SIZE_ADDRESS_ARR, 0);

    return BDMF_ERR_OK;
}

int rdd_ag_us_tm_bbh_tx_fifo_size_get(uint8_t *bits)
{
    RDD_BYTE_1_BITS_READ_G(*bits, RDD_BBH_TX_FIFO_SIZE_ADDRESS_ARR, 0);
