...
void dump_RDD_CSO_CONTEXT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\n");
	FIELD_MREAD_32((uint8_t *)p + 104, 8, 16, r);
	bdmf_session_print(session, "\tsaved_buffer_number      = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	{ uint32_t temp; FIELD_MREAD_32(((uint8_t *)p + 104), 0, 8, temp); r = temp << 8; FIELD_MREAD_32(((uint8_t *)p + 108), 24, 8, temp); r = r | temp; };
...
void dump_RDD_PACKET_BUFFER(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\n");
	bdmf_session_print(session, "\tscratch_ext              =\n\t");
	for (i=0,j=0; i<32; i++)
	{
		MREAD_I_8((uint8_t *)p + 288, i, r);
		bdmf_session_print(session, "0x%08x ", (unsigned int)r);
		j++;
		if (j >= 8)
		{
...
void dump_RDD_DHD_HW_CONFIGURATION(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\n");
	FIELD_MREAD_32((uint8_t *)p + 16, 24, 16, r);
	bdmf_session_print(session, "\treserved2                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_TCPSPDTEST_ENGINE_REF_PKT_HDR(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	int i,j;

	bdmf_session_print(session, "  Register TCPSPDTEST_ENGINE_REF_PKT_HDR\n");
	...
	MREAD_32((uint8_t *)p + 4, r);
	bdmf_session_print(session, "\tpadding                  = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	bdmf_session_print(session, "\thdr                      =\n\t");
	for (i=0,j=0; i<118; i++)
	{
		MREAD_I_8((uint8_t *)p + 8, i, r);
		bdmf_session_print(session, "0x%08x ", (unsigned int)r);
...
void dump_RDD_TCPSPDTEST_ENGINE_TCB(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	int i,j;

	bdmf_session_print(session, "  Register TCPSPDTEST_ENGINE_TCB\n");

	MREAD_32((uint8_t *)p, r);
	bdmf_session_print(session, "\trx_pkts                  = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 4, r);
	bdmf_session_print(session, "\ttxed_pkts                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 8, r);
	bdmf_session_print(session, "\tfreed_pkts               = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 12, r);
	bdmf_session_print(session, "\tack_seq                  = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 16, r);
	bdmf_session_print(session, "\ttx_seq                   = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 20, r);
	bdmf_session_print(session, "\tbad_pkts                 = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 24, r);
	bdmf_session_print(session, "\tno_credits               = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	bdmf_session_print(session, "\tbad_bytes                =\n\t");
	for (i=0,j=0; i<2; i++)
	{
	...
	bdmf_session_print(session, "\n");
	bdmf_session_print(session, "\trx_bytes                 =\n\t");
	for (i=0,j=0; i<2; i++)
	{
		MREAD_I_32((uint8_t *)p + 40, i, r);
		bdmf_session_print(session, "0x%08x ", (unsigned int)r);
		j++;
	}
	bdmf_session_print(session, "\n");
	bdmf_session_print(session, "\texpected_bytes           =\n\t");
	for (i=0,j=0; i<2; i++)
	{
		MREAD_I_32((uint8_t *)p + 48, i, r);
		bdmf_session_print(session, "0x%08x ", (unsigned int)r);
...
void dump_RDD_TCPSPDTEST_ENGINE_PKT_DROP(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	int i,j;

	bdmf_session_print(session, "  Register TCPSPDTEST_ENGINE_PKT_DROP\n");

	MREAD_8((uint8_t *)p, r);
	bdmf_session_print(session, "\tis_drop                  = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 1, r);
	bdmf_session_print(session, "\tis_win_full              = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)p + 2, r);
	bdmf_session_print(session, "\tnum_errs                 = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)p + 4, r);
	bdmf_session_print(session, "\twr_ofs                   = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)p + 6, r);
	bdmf_session_print(session, "\trd_ofs                   = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	bdmf_session_print(session, "\tseq                      =\n\t");
	for (i=0,j=0; i<128; i++)
	{
...
void dump_RDD_TCPSPDTEST_ENGINE_CONN_INFO(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register TCPSPDTEST_ENGINE_CONN_INFO\n");

	MREAD_8((uint8_t *)p, r);
	bdmf_session_print(session, "\tl2_hdr_len               = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 1, r);
	bdmf_session_print(session, "\tl3_hdr_len               = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 2, r);
	bdmf_session_print(session, "\ttx_pd_len                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 3, r);
	bdmf_session_print(session, "\tcpu_rx_rdd_queue         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	MREAD_16((uint8_t *)p + 6, r);
	bdmf_session_print(session, "\ttx_qm_queue              = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_SYSTEM_CONFIGURATION_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\tgeneric_miss_traffic     = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 4, 2, 1, r);
	bdmf_session_print(session, "\tgeneric_all_traffic      = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
