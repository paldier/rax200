...
int rdd_ag_ds_tm_vlan_tx_counters_packets_set(uint32_t _entry, uint32_t packets)
{
    if(_entry >= RDD_VLAN_TX_COUNTERS_SIZE)
         return BDMF_ERR_PARM;

    RDD_PACKETS_AND_BYTES_PACKETS_WRITE_G(packets, RDD_VLAN_TX_COUNTERS_ADDRESS_ARR, _entry);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_vlan_tx_counters_packets_get(uint32_t _entry, uint32_t *packets)
{
    if(_entry >= RDD_VLAN_TX_COUNTERS_SIZE)
         return BDMF_ERR_PARM;

    RDD_PACKETS_AND_BYTES_PACKETS_READ_G(*packets, RDD_VLAN_TX_COUNTERS_ADDRESS_ARR, _entry);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_vlan_tx_counters_bytes_set(uint32_t _entry, uint32_t bytes)
{
    if(_entry >= RDD_VLAN_TX_COUNTERS_SIZE)
         return BDMF_ERR_PARM;

    RDD_PACKETS_AND_BYTES_BYTES_WRITE_G(bytes, RDD_VLAN_TX_COUNTERS_ADDRESS_ARR, _entry);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_vlan_tx_counters_bytes_get(uint32_t _entry, uint32_t *bytes)
{
    if(_entry >= RDD_VLAN_TX_COUNTERS_SIZE)
         return BDMF_ERR_PARM;

    RDD_PACKETS_AND_BYTES_BYTES_READ_G(*bytes, RDD_VLAN_TX_COUNTERS_ADDRESS_ARR, _entry);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_g9991_control_sid_table_set(uint32_t _entry, uint32_t bits)
{
    if(_entry >= RDD_G9991_CONTROL_SID_TABLE_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTES_4_BITS_WRITE_G(bits, RDD_G9991_CONTROL_SID_TABLE_ADDRESS_ARR, _entry);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_g9991_control_sid_table_get(uint32_t _entry, uint32_t *bits)
{
    if(_entry >= RDD_G9991_CONTROL_SID_TABLE_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTES_4_BITS_READ_G(*bits, RDD_G9991_CONTROL_SID_TABLE_ADDRESS_ARR, _entry);
