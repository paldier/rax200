...
void dump_RDD_BYTES_16(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	int i,j;

	bdmf_session_print(session, "  Register BYTES_16\n");
...
void dump_RDD_DS_TM_EGRESS_PORT_RR_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register DS_TM_EGRESS_PORT_RR_ENTRY\n");

	MREAD_8((uint8_t *)p, r);
	bdmf_session_print(session, "\tnext_sched               = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_BROADCOM_SWITCH_PORT_MAPPING(bdmf_session_handle session, unsigned char *p)
{
	...
	MREAD_16((uint8_t *)p, r);
	bdmf_session_print(session, "\trx_flow_table_index      = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_POST_LOOKUP_CFG_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register POST_LOOKUP_CFG_ENTRY\n");
	...
	MREAD_16((uint8_t *)p + 2, r);
	bdmf_session_print(session, "\tip_addresses_table_addr  = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
