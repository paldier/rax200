...
{
    if(!scheduling_queue_descriptor || _entry >= RDD_US_TM_SCHEDULING_QUEUE_TABLE_SIZE || scheduling_queue_descriptor->bbh_queue_index >= 64)
          return BDMF_ERR_PARM;
...
{
    RDD_BYTE_1_BITS_WRITE_G(bits, RDD_US_TM_ETH_BBH_TX_FIFO_SIZE_ADDRESS_ARR, 0);
...
{
    RDD_BYTE_1_BITS_READ_G(*bits, RDD_US_TM_ETH_BBH_TX_FIFO_SIZE_ADDRESS_ARR, 0);

    return BDMF_ERR_OK;
}

int rdd_ag_us_tm_dsl_bbh_tx_fifo_size_set(uint8_t bits)
{
    RDD_BYTE_1_BITS_WRITE_G(bits, RDD_US_TM_DSL_BBH_TX_FIFO_SIZE_ADDRESS_ARR, 0);

    return BDMF_ERR_OK;
}

int rdd_ag_us_tm_dsl_bbh_tx_fifo_size_get(uint8_t *bits)
{
    RDD_BYTE_1_BITS_READ_G(*bits, RDD_US_TM_DSL_BBH_TX_FIFO_SIZE_ADDRESS_ARR, 0);
...
{
    RDD_BYTE_1_BITS_WRITE_G(bits, RDD_US_TM_PON_BBH_TX_FIFO_SIZE_ADDRESS_ARR, 0);
...
{
    RDD_BYTE_1_BITS_READ_G(*bits, RDD_US_TM_PON_BBH_TX_FIFO_SIZE_ADDRESS_ARR, 0);
