...
void _rdd_fc_context_read_from_table_entry(rdd_fc_context_t *ctx, RDD_FLOW_CACHE_CONTEXT_ENTRY_DTS *entry)
{
    ...
    RDD_FLOW_CACHE_CONTEXT_ENTRY_GRE_TUNNEL_INDEX_READ(ctx->gre_tunnel_index, entry);
    ...
    bdmf_boolean is_add)
{
    ...
    if (( ctx->actions_vector & rdpa_fc_act_forward ) && ( ctx->fwd_action == RDD_FLOW_CACHE_FORWARD_ACTION_CPU))
    {
        ctx->trap_reason = ctx->trap_reason - rdpa_cpu_rx_reason_udef_0;
    ...
    RDD_FLOW_CACHE_CONTEXT_ENTRY_GRE_TUNNEL_INDEX_WRITE(ctx->gre_tunnel_index, entry);
                                                  ...
                                                  rdpa_traffic_dir                 xi_direction )
{
    ...
    if ( xi_add_connection->lookup_entry->dst_ip.family != bdmf_ip_family_ipv4 )
    {
        ...
        {
            if ( xi_add_connection->context_entry.actions_vector & ( rdpa_fc_action_dslite_tunnel || rdpa_fc_action_l2gre_tunnel ) )
            {
                return ( BL_LILAC_RDD_ERROR_ADD_CONTEXT_ENTRY );
            }
        }
