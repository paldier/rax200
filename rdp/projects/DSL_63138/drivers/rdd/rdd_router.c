                                                          ...
                                                          uint32_t is_new_entry )
{
    ...
    if( xi_context_entry->fc_ucast_flow_context_entry.multicast_flag == 0 )
    {
        ...
        RDD_FC_UCAST_FLOW_CONTEXT_ENTRY_IS_HIGH_PRIO_WRITE( xi_context_entry->fc_ucast_flow_context_entry.is_high_prio, xi_context_entry_ptr );
    }
    else
    {
    ...
    if( !multicast_flag )
    {
        ...
        if ( valid )
        {
            ...
            if ( xi_entry_index == context_index )
            {
                ...
                RDD_FC_UCAST_FLOW_CONTEXT_ENTRY_IS_HIGH_PRIO_READ( xo_context_entry->fc_ucast_flow_context_entry.is_high_prio, context_entry_ptr );
