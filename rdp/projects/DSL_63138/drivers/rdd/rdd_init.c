...
static BL_LILAC_RDD_ERROR_DTE f_rdd_local_registers_initialize ( uint8_t lp_mode );
...
#if defined(DSL_63138) && defined(DSL_63148) && !defined(RDD_BASIC)
    /* DDR reset */
...
//    MEMSET ( ( void * )g_fc_mcast_connection2_table_ptr, 0, sizeof ( RDD_FC_MCAST_CONNECTION2_TABLE_DTS ) );
...
#else
    /* initialize the base address of the BPM base address */
    f_rdd_bpm_initialize(VIRT_TO_PHYS(g_runner_ddr_base_addr), 0, VIRT_TO_PHYS(g_runner_extra_ddr_base_addr));

    /* initialize runner dma base address */
    f_rdd_ddr_initialize(VIRT_TO_PHYS(g_runner_ddr_base_addr), 0, g_ddr_headroom_size);
#endif
    ...
    f_rdd_local_registers_initialize (init_params->lp_mode);
...
static BL_LILAC_RDD_ERROR_DTE f_rdd_local_registers_initialize ( uint8_t lp_mode )
{
    ...
    local_register[ WAN_FILTERS_AND_CLASSIFICATION_THREAD_NUMBER ][ CS_R14 ] = (lp_mode) ? COUNTDOWN_LOOP_VALUE : 0;
    ...
    local_register[ WAN1_FILTERS_AND_CLASSIFICATION_THREAD_NUMBER ][ CS_R14 ] = (lp_mode) ? COUNTDOWN_LOOP_VALUE : 0;

#if defined(WL4908)
    /* RX Buffer Copy */
    local_register[ DS_RX_BUFFER_COPY_THREAD_NUMBER ][ CS_R16 ] = ADDRESS_OF(runner_a, rx_buffer_copy_wakeup_request) << 16;
    local_register[ DS_RX_BUFFER_COPY_THREAD_NUMBER ][ CS_R8  ] = GPON_RX_NORMAL_DESCRIPTORS_ADDRESS;
    local_register[ DS_RX_BUFFER_COPY_THREAD_NUMBER ][ CS_R10 ] = INGRESS_HANDLER_BUFFER_ADDRESS | ( CPU_REASON_WAN1_TABLE_INDEX << 16 );
    local_register[ DS_RX_BUFFER_COPY_THREAD_NUMBER ][ CS_R19 ] = ( BBH_PERIPHERAL_SBPM + SBPM_REPLY_SET_0 ) | (( SBPM_REPLY_ADDRESS + SBPM_REPLY_GET_NEXT_OFFSET + SBPM_REPLY_SET_0_OFFSET ) << 16 );
    local_register[ DS_RX_BUFFER_COPY_THREAD_NUMBER ][ CS_R20 ] = WAN1_FILTERS_AND_CLASSIFICATION_THREAD_WAKEUP_REQUEST_VALUE;
    local_register[ DS_RX_BUFFER_COPY_THREAD_NUMBER ][ CS_R21 ] = ( DS_RX_SBPM_TO_FPM_COPY_FPM_ALLOC_RESULT_TABLE_ADDRESS ) | ( DS_MCAST_RX_SBPM_TO_FPM_COPY_FPM_ALLOC_RESULT_TABLE_ADDRESS << 16 );
#endif
    ...
    local_register[ SERVICE_QUEUE_DEQUEUE_THREAD_NUMBER - 32 ][ CS_R10 ] = ( DOWNSTREAM_LAN_ENQUEUE_SQ_PD_ADDRESS << 16 ) | DHD_TX_POST_PD_INGRESS_QUEUE_ADDRESS;
    ...
    local_register[ LAN_DISPATCH_THREAD_NUMBER - 32 ][ CS_R14 ] = 0; /* lp_mode is not configured for WL4908 */
    ...
    local_register[ LAN1_DISPATCH_THREAD_NUMBER - 32 ][ CS_R14 ] = 0; /* lp_mode is not configured for WL4908 */
    ...
    local_register[ LAN2_DISPATCH_THREAD_NUMBER - 32 ][ CS_R14 ] = 0; /* lp_mode is not configured for WL4908 */
...
#else
    local_register[ UPSTREAM_FLOW_CACHE_SLAVE0_THREAD_NUMBER - 32 ][ CS_R8  ] = (uint32_t) ((US_CONNECTION_BUFFER_TABLE_ADDRESS + 0 * RDD_US_CONNECTION_BUFFER_TABLE_SIZE2 * sizeof(RDD_CONNECTION_ENTRY_DTS)) << 16);
#endif
...
#else
    local_register[ UPSTREAM_FLOW_CACHE_SLAVE1_THREAD_NUMBER - 32 ][ CS_R8  ] = (uint32_t) ((US_CONNECTION_BUFFER_TABLE_ADDRESS + 1 * RDD_US_CONNECTION_BUFFER_TABLE_SIZE2 * sizeof(RDD_CONNECTION_ENTRY_DTS)) << 16);
#endif
...
#else
    local_register[ UPSTREAM_FLOW_CACHE_SLAVE2_THREAD_NUMBER - 32 ][ CS_R8  ] = (uint32_t) ((US_CONNECTION_BUFFER_TABLE_ADDRESS + 2 * RDD_US_CONNECTION_BUFFER_TABLE_SIZE2 * sizeof(RDD_CONNECTION_ENTRY_DTS)) << 16);
#endif
...
#else
    local_register[ UPSTREAM_FLOW_CACHE_SLAVE3_THREAD_NUMBER - 32 ][ CS_R8  ] = (uint32_t) ((US_CONNECTION_BUFFER_TABLE_ADDRESS + 3 * RDD_US_CONNECTION_BUFFER_TABLE_SIZE2 * sizeof(RDD_CONNECTION_ENTRY_DTS)) << 16);
#endif
