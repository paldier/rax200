...
#if (defined(CONFIG_BCM96838) || defined(CONFIG_BCM96848) || defined(CONFIG_BCM96858) || defined(CONFIG_BCM96846) || defined(CONFIG_BCM96836)) && !defined(BDMF_SYSTEM_SIM)
...
#if defined(BDMF_SYSTEM_SIM)
static int misc_runner_memory_dump_command(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...
static int misc_runner_segs_dump_command(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    tx_pointers_tbl_size = sizeof(uint16_t) * MAX_NUM_OF_WAN_CHANNELS *
        RDD_WAN_CHANNEL_0_7_DESCRIPTOR_RATE_CONTROLLER_ADDR_NUMBER * RDD_US_RATE_CONTROLLER_DESCRIPTOR_TX_QUEUE_ADDR_NUMBER;
    ...
    for (rnr_idx = 0; rnr_idx < RNR_MEM_BLOCK.addr_count; rnr_idx++)
    {
        ...
        if (segment_file_init(file_name, "w+b", (RU_BLK(RNR_MEM).addr[rnr_idx] + RU_REG_OFFSET(RNR_MEM, HIGH)), mem_high_byte_num * 2))
            goto exit;
        sprintf(file_name, "context_segment_%d", rnr_idx);
        if (segment_file_init(file_name, "w+b", (RU_BLK(RNR_CNTXT).addr[rnr_idx] + RU_REG_OFFSET(RNR_CNTXT, MEM_ENTRY)), mem_cntxt_byte_num))
            goto exit;
    }
    if (rdd_sim_save_ddr_tables())
        goto exit;
...
static int misc_mdio_22_read_register_command(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    uint8_t parameter_index = 0;
...
static int misc_mdio_22_write_register_command(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    uint8_t parameter_index = 0;
    misc_mdio_type_t type = parm[parameter_index++].value.unumber; /* input */
    uint32_t phy = parm[parameter_index++].value.unumber; /* input */
...
static int misc_mdio_45_read_register_command(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    uint8_t parameter_index = 0;
    misc_mdio_type_t type = parm[parameter_index++].value.unumber; /* input */
    uint32_t phy = parm[parameter_index++].value.unumber; /* input */
...
static int misc_mdio_45_write_register_command(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    uint8_t parameter_index = 0;
    misc_mdio_type_t type = parm[parameter_index++].value.unumber; /* input */
    uint32_t phy = parm[parameter_index++].value.unumber; /* input */
...
#if defined(CONFIG_BCM96848)
static int misc_onu2g_read_register_command(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    uint8_t parameter_index = 0;
    uint32_t addr = parm[parameter_index++].value.unumber; /* input */
    uint16_t value;
...
static int misc_onu2g_write_register_command(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    uint8_t parameter_index = 0;
    uint32_t addr = parm[parameter_index++].value.unumber; /* input */
    uint32_t value = parm[parameter_index++].value.unumber; /* input */
    ...
    uint16_t n_parms)
{
    ...
    bdmf_session_print(session, "== Result:\n\t"
        ...
        "aggregation_en = %d, arl_lookup_method = %d, counter_id=%d, counter_id_valid=%d\n",
    ...
    uint16_t n_parms)
{
    ...
    bridge_and_vlan_lkp_result.wan_vid = (int16_t)parm[parm_idx++].value.number;
    ...
    bridge_and_vlan_lkp_result.arl_lookup_method = (uint8_t)parm[parm_idx++].value.unumber;
...
void misc_shell_init(void)
{
    ...
    BDMFMON_MAKE_CMD(misc_dir, "wvh", "Write VLAN Hash entry", misc_vlan_hash_entry_handler_write,
        ...
        BDMFMON_MAKE_PARM_RANGE("wan_vid", "WAN VID", BDMFMON_PARM_NUMBER, 0, 0, 4095),
        ...
        BDMFMON_MAKE_PARM_RANGE("arl_lkp", "ARL Lookup method (0-mac, 1-mac+vid)", BDMFMON_PARM_NUMBER, 0, 0, 1),
