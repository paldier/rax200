...
BL_LILAC_RDD_ERROR_DTE f_rdd_connection_table_initialize ( void )
{
    ...
    nat_cache_table_address = RDD_VIRT_TO_PHYS( NatCacheTableBase );
#else
...
#if !defined(BDMF_SYSTEM_SIM)
    context_table_address = RDD_VIRT_TO_PHYS( ContextTableBase );
#else
...
#if !defined(BDMF_SYSTEM_SIM)
    context_cont_table_address = RDD_VIRT_TO_PHYS( ContextContTableBase );
#else
        ...
        for ( i = 0; i < RDD_FC_NATC_MCAST_FLOW_CONTEXT_ENTRY_PORT_CONTEXT_NUMBER; i++ )
        {
            ...
            if(old_port_mask ^ new_port_mask)
            {
                ...
                else
                {
                    ...
                    RDD_FC_MCAST_PORT_CONTEXT_ENTRY_IS_HIGH_PRIO_WRITE(port_context_ptr->is_high_prio, &port_context);
    ...
    else
    {
        ...
        if ( valid )
        {
            ...
            for ( i = 0; i < RDD_FC_MCAST_FLOW_CONTEXT_ENTRY_PORT_CONTEXT_NUMBER; i++ )
            {
                ...
                RDD_FC_MCAST_PORT_CONTEXT_ENTRY_IS_HIGH_PRIO_READ(port_context_ptr->is_high_prio, &port_context);
                                                          ...
                                                          uint16_t                *connection2_entry_index_res )
{
    ...
    __debug_mcast("\n%s, %u: Input Params: vlan_head_index=%u, vid0=0x%03X vid1=0x%03X\n", __FUNCTION__, __LINE__, 
              vlan_head_index, mcast_lookup_entry->outer_vlan_id, mcast_lookup_entry->inner_vlan_id);
                                           ...
                                           bdmf_index     *entry_index )
{
    ...
    uint32_t                  ipv6_dst_ip_crc;
    uint32_t                  ip_sa;
    rdpa_mcast_flow_key_t     *mcast_lookup_entry = (rdpa_mcast_flow_key_t *) get_connection->lookup_entry;
    RDD_FC_MCAST_CONNECTION2_ENTRY_DTS  *mcast_connection2_entry_ptr;
    uint16_t                  connection2_entry_index ;
    uint16_t                  vlan_head_index;
    uint32_t                  ssm=0;
...
int rdd_fc_mcast_connection_entry_add ( rdd_ip_flow_t  *add_connection )
{
    ...
    add_connection->context_entry.fc_mcast_flow_context_entry.connection_table_index = master_nat_cache_entry_index;
    add_connection->context_entry.fc_mcast_flow_context_entry.connection_direction = rdpa_dir_ds;
    add_connection->context_entry.fc_mcast_flow_context_entry.multicast_flag = 1;
    ...
    {   /*Primary entry*/
        ...
        RDD_FC_MCAST_CONNECTION_ENTRY_COMMAND_LIST_LENGTH_64_WRITE( add_connection->context_entry.fc_mcast_flow_context_entry.command_list_length_64, nat_cache_lkp_entry_ptr );
