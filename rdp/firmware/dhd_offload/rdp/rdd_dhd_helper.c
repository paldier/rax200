...
#ifndef U_LINUX
#if defined(WL4908)
    uint32_t *threshold_ptr = (uint32_t *)(DEVICE_ADDRESS(RUNNER_COMMON_0_OFFSET) + DHD_TX_POST_BUFFERS_THRESHOLD_ADDRESS);

    /* The tx post buffers threshold is global for all radios. */
    /* 4908's threshold setting is 3/4 of total available token. which is 3/4 * (2 * 64K) = 96k */
    MWRITE_32((uint8_t *)threshold_ptr, (96 * 1024));
#else
...
void rdd_dhd_helper_flow_ring_disable(uint32_t radio_idx, uint32_t flow_ring_idx)
{
    ...
    bdmf_fastlock_lock_irq(&int_lock_irq, flags);
    rdd_cpu_tx_send_message(RDD_CPU_TX_MESSAGE_DHD_MESSAGE, PICO_RUNNER_A, RUNNER_PRIVATE_0_OFFSET,
        DHD_MSG_TYPE_FLOW_RING_SET_DISABLED, flow_ring_idx | (radio_idx << 14), 0, 1);
    bdmf_fastlock_unlock_irq(&int_lock_irq, flags);
}
