...
GFWPT_ENABLE="`nvram get fwpt_enable`"

echo "Start agApi_fwSetTriggerConfByNvram.sh"
echo "fwpt_enable = "$GFWPT_ENABLE
...
do
    ...
    # only enter enter the rules when fwpt_enable is 1
    if [ "$GFWPT_ENABLE" != "1" ]; then
        echo "fwpt_enable != 1 exit"
        break;
    fi
    ...
    # tcp or udp
        ...
        RET=`iptables -t nat -A PORTTRIGGER_NAT_CHAIN -i ${WAN_IFNAME} -p ${FWPT_MATCH_PROTO_STR} --dport ${FWPT_TRIGGER_SPORT}:${FWPT_TRIGGER_DPORT} -j TRIGGER --trigger-type dnat`
