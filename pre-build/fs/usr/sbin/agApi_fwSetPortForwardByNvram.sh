...
for i in `seq 1 $FWPF_RULE`
do
    FWPF_RULE_NUM="`nvram get inbound_policy_tbl$i`"
    FWPF_PROTO="`echo $FWPF_RULE_NUM | cut -d';' -f2`"
    FWPF_EX_SPORT="`echo $FWPF_RULE_NUM | cut -d';' -f3`"
    FWPF_EX_EPORT="`echo $FWPF_RULE_NUM | cut -d';' -f4`"
    FWPF_IN_IPADDR="`echo $FWPF_RULE_NUM | cut -d';' -f5`"
    FWPF_IN_SPORT="`echo $FWPF_RULE_NUM | cut -d';' -f6`"
    FWPF_IN_EPORT="`echo $FWPF_RULE_NUM | cut -d';' -f7`"

    echo FWPF_RULE_NUM=$FWPF_RULE_NUM
