...
static uint8_t  sf2_hw_switching_state = HW_SWITCHING_DISABLED;
...
static uint32_t wanQueMap = DefaultWANQueBitMap,
                maxStreamNumber = MaxStreamNumber, curStreams, queThreConfMode;
...
#define PER_QUE_OVERSUB_FACTOR 2/100
    ...
    /*
       ...
       below hard limits.
     */
    available_switch_buf -= acb_xoff_threshold * curStreams;
     ...
     */
    thredBase[FC_THRED_TTL_PAUSE_TYPE] = thredBase[FC_THRED_TTL_DROP_TYPE] - maxFrameLength * curStreams;
    ...
    /* Set per-queue drop threshold to half of total drop threshold */
    thredBase[FC_THRED_QUE_DROP_TYPE] = thredBase[FC_THRED_TTL_DROP_TYPE]/2;

    /* Set per-queue pause threshould one frame size lower than per-q drop threshold */
    thredBase[FC_THRED_QUE_PAUSE_TYPE] = thredBase[FC_THRED_QUE_DROP_TYPE] - maxFrameLength;
...
rgmii_registers_t rgmii_port_reg_offset [] = {
...
#else
                OFFSETOF(TopControl, xMIIPadCtrl[1]), 11},
#endif
             ...
             {  OFFSETOF(EthernetSwitchReg, rgmii_12_ctrl), 
                ...
                OFFSETOF(Misc, miscxMIIPadCtrl[0]), 12},
#else
                OFFSETOF(TopControl, xMIIPadCtrl[0]), 12},
#endif

             {  OFFSETOF(EthernetSwitchReg, rgmii_13_ctrl), 
                OFFSETOF(EthernetSwitchReg, rgmii_13_rx_clk_delay_ctrl), 
#if (CONFIG_BRCM_CHIP_REV==0x63158A0)
                OFFSETOF(Misc, miscxMIIPadCtrl[2]), 13},
#else
                OFFSETOF(TopControl, xMIIPadCtrl[2]), 13},
#endif
#endif
};
...
void _sf2_conf_acb_conges_profile(int profile)
{
    ...
    *sf2_acb_xon_thresh = (p->total_xon_hyst & SF2_ACB_BUFS_THRESH_M) << SF2_ACB_TOTAL_XON_BUFS_S |
    ...
    acb_xoff_threshold = (ENET_MAX_MTU_PAYLOAD_SIZE + SF2_PKT_BUF_SZ - 1) / SF2_PKT_BUF_SZ;  // get number of buffers needed
...
int tr_enet_phy_pbmap(enetx_port_t *port, void *ctx)
{
    int *phy_map = ctx;
...
static int enet_get_phy_enet_pbmap(enetx_port_t *sw)
{
    int phy_map = 0;
    port_traverse_ports(sw, tr_enet_phy_pbmap, PORT_CLASS_PORT, &phy_map);
    return phy_map;
}
...
int ioctl_extsw_info(struct ethswctl_data *e)
{
    ...
    e->port_map = info->sw.port_map & ~enet_get_phy_enet_pbmap(sw);
            ...
            case 100:   // unpublished debug type: dump fc_peak_tx_q[0..63]
                {
                    ...
                    for (i = 0; i < 8; i++)
                    {
                        ...
                        for (j = 0; j < 8; j++)
                        {
                            val = *(sf2_acb_que_in_flight + i*8 + j);
                            printk("%3d", val & 0x7ff);
                        }
                        printk("|");
#endif
                        ...
                        for (j = 0; j < 8; j++)
                        {
                            SF2SW_RREG(PAGE_FLOW_CTRL_XTN, REG_FC_QUE_CUR_COUNT+j*2, &val, 4);
                            printk("%3d", val & 0x7ff);
                        }
                        printk("|");
                        // dump peak tx regs
                        ...
                        SF2SW_RREG(PAGE_FLOW_CTRL_XTN, REG_FC_SYS_TOTAL_PEAK_COUNT, &val, 4);
                        printk(" %3d\n", val & 0x7ff);
                    }
...
static char *print_phy_position(phy_dev_t *phy_dev)
{
    static char buf[128];
    char *bf = buf;

    if (IsExtPhyId(phy_dev->meta_id) && !IsSerdes(phy_dev->meta_id))
        bf += sprintf(bf, "External ");
    else
        bf += sprintf(bf, "On-chip ");

    if (phy_dev->cascade_prev)
        bf += sprintf(bf, "Cascaded ");

    if (IsRGMII(phy_dev->meta_id))
        bf += sprintf(bf, "RGMII ");
    else if (IsC45Phy(phy_dev))
        bf += sprintf(bf, "Claus45 ");

    sprintf(bf, "%s", phy_dev->phy_drv->name);
    ...
    for (j = 0; j < 2; j++)
    {
        for (i = 0; i < sw->s.port_count; i++)
        {
            ...
            if (j == 0)    /* Parent round */
            {
                ...
                if (phy_is_crossbar(p->p.phy))
                {
                    if (cat_snprintf(buf, sz, "%s: <%s sw port: %d> <Logical : %02d> MAC : %02X:%02X:%02X:%02X:%02X:%02X\n",
                            dev->name, (IS_ROOT_SW(sw)?"Int":"Ext"), p->p.mac->mac_id, PHYSICAL_PORT_TO_LOGICAL_PORT(p->p.mac->mac_id, IS_ROOT_SW(sw)?0:1),
                            dev->dev_addr[0], dev->dev_addr[1], dev->dev_addr[2], dev->dev_addr[3], dev->dev_addr[4], dev->dev_addr[5]) == 0)
                            return 0;

                    for (phy = crossbar_phy_dev_first(p->p.phy); phy; phy = crossbar_phy_dev_next(phy))
                    {
                        int ext_idx = crossbar_external_endpoint(phy);
                        if (cat_snprintf(buf, sz, "    Chip Physical Port %2d, Cross Bar Port %d, PHY_ID <0x%08x:0x%02x:%s >\n",
                                BP_CROSSBAR_PORT_TO_PHY_PORT(ext_idx),  ext_idx,
                                phy->meta_id, phy->addr, print_phy_position(phy)) == 0)
                            return 0;

                        if (phy->cascade_next && !(phy->cascade_next->flag & PHY_FLAG_DYNAMIC))
                        {
                            if (cat_snprintf(buf, sz, "                                             PHY_ID <0x%08x:0x%02x:%s>\n",
                                    phy->cascade_next->meta_id, phy->cascade_next->addr, print_phy_position(phy->cascade_next)) == 0)
                                return 0;
                        }
                    }
                }
                else
                {
                    ...
                    if (phy->phy_drv && phy->phy_drv->phy_type == PHY_TYPE_XGAE) {
                        if(cat_snprintf(buf, sz, "%s: <%s sw port: %d> <Logical : %02d> PHY_ID <N/A:On-Chip 10G Active Ethernet Serdes> MAC:%02X:%02X:%02X:%02X:%02X:%02X\n",
                                dev->name, (IS_ROOT_SW(sw)?"Int":"Ext"), p->p.mac->mac_id, PHYSICAL_PORT_TO_LOGICAL_PORT(p->p.mac->mac_id, IS_ROOT_SW(sw)?0:1),
                                dev->dev_addr[0], dev->dev_addr[1], dev->dev_addr[2], dev->dev_addr[3], dev->dev_addr[4], dev->dev_addr[5]) == 0)
                            return 0;
                    }
                    else
                        if (cat_snprintf(buf, sz, "%s: <%s sw port: %d> <Logical : %02d> PHY_ID <0x%08x:0x%02x:%s> MAC:%02X:%02X:%02X:%02X:%02X:%02X\n",
                                dev->name, (IS_ROOT_SW(sw)?"Int":"Ext"), p->p.mac->mac_id, PHYSICAL_PORT_TO_LOGICAL_PORT(p->p.mac->mac_id, IS_ROOT_SW(sw)?0:1),
                                phy->meta_id, phy->addr, print_phy_position(phy),
                                dev->dev_addr[0], dev->dev_addr[1], dev->dev_addr[2], dev->dev_addr[3], dev->dev_addr[4], dev->dev_addr[5]) == 0)
                            return 0;
...
#endif
        self->port_class = PORT_CLASS_PORT;
        rtnl_lock();
        enet_create_netdevice(self);
        rtnl_unlock();
        self->port_class = PORT_CLASS_PORT_UNDEF;
...
int enetxapi_post_config(void)
{
    ...
    port_traverse_ports(sf2_sw, tr_reassign_undef_ports, PORT_CLASS_PORT_UNDEF, NULL);
...
// =========== sf2 switch ops =============================
static void port_sf2_sw_fast_age(enetx_port_t *sw)
{
    //enet_dbgv(" sw=%s\n", sw->obj_name);
...
static int port_sf2_sw_init(enetx_port_t *self)
{
   ...
   /* Low Perfornce Runner cannot handle packet rate >400kpps */
   if (UtilGetChipIsLP())
   {
...
static void port_sf2_sw_stats_clear(enetx_port_t *self)
{
...
// =========== sf2 port ops =============================
extern bdmf_object_handle create_rdpa_port(rdpa_if rdpaif, rdpa_emac emac, bdmf_object_handle owner, rdpa_if high_prio_sid);
...
int port_sf2_port_init(enetx_port_t *self)
{
    ...
    if (mux_set_rx_index(self->p.parent_sw, rdpaif, self))
        return -1;
...
static void port_sf2_fast_age(enetx_port_t *port)
{
    uint8_t ctrl;
...
//  .mux_port_tx = port_sf2_sw_mux,
    ...
    .stats_clear = port_sf2_sw_stats_clear,
