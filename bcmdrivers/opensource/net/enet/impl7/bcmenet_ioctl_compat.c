...
static int _handle_gpon(struct ifreq *rq)
{
    ...
    static unsigned char gem_map_arr[CONFIG_BCM_MAX_GEM_PORTS];
    ...
    switch (gpn.op)
    {
        ...
        case DELETEGPONVPORT:
            for (i = 0; i < CONFIG_BCM_MAX_GEM_PORTS; i++)
            {
                if (gem_map_arr[i])
                {
                    enet_err("gpon gem still in use %d\n", i);
                    return -1;
                }
            }
            memset(gem_map_arr, 0, CONFIG_BCM_MAX_GEM_PORTS);
        case DELETEALLGPONVPORTS:
            if (!gpon_port)
                return -ENOENT;

            /* At high traffic, packets will still be queued up in rxq with src_port 0 which needs to be ignored.
             * Workaround: delay a bit to allow rxq to be processed.. */
            msleep_interruptible(100);
            sw_free(&gpon_port);
            return 0;

        case SETGEMIDMAP:
            memcpy(gem_map_arr, gpn.gem_map_arr, sizeof(gem_map_arr));
            return 0;
        case GETFREEGEMIDMAP:
        case GETGEMIDMAP:
            memcpy(gpn.gem_map_arr, gem_map_arr, sizeof(gem_map_arr));
            if (copy_to_user((void *)rq->ifr_data, (void *)&gpn, sizeof(gpn)))
                return -EFAULT;
...
static int _handle_siocgmiiphy(enetx_port_t *self, struct mii_ioctl_data *mii)
{
    ...
    if (mii->val_in != 0xffff)
        phy_dev = crossbar_group_phy_get(self->p.phy, BP_PHY_PORT_TO_CROSSBAR_PORT(mii->val_in));
    else
        phy_dev = self->p.phy;
    phy_id = phy_dev->meta_id;

    if (phy_dev->phy_drv->phy_type == PHY_TYPE_MAC2MAC)
        return -EINVAL;

    mii->phy_id = (uint16_t)phy_id;
    /* store phy index in val_out */
    mii->val_out = phy_dev_get_uid(phy_dev) << ETHCTL_PHY_UID_SHIFT_IN_VAL_OUT;
...
int ioctl_handle_mii(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    // if uid was specified, get phy based on uid, otherwise use active phy associated with dev
    phy = phy_dev_get_by_uid(ETHCTL_GET_PHY_UID_FROM_VAL_OUT(mii->val_out));
    if (!phy || !phy->phy_drv)
        phy = port->p.phy;
...
static phy_dev_t* enet_dev_get_phy_dev(struct net_device *dev, uint32_t phy_addr)
{
    enetx_port_t *port = ((enetx_netdev *)netdev_priv(dev))->port;
    phy_dev_t *phy;

    // if uid was specified, get phy based on uid, otherwise use active phy associated with dev
    phy = phy_dev_get_by_uid(ETHCTL_GET_PHY_UID_FROM_VAL_OUT(phy_addr));
    if (!phy || !phy->phy_drv)
        phy = port->p.phy;
...
mapFail:
    ...
    case ETHMOVESUBPORT:
        {
            ...
            if (port == root_sw) {  // get next interface-phy mapping
                ...
                info.external_endpoint = BP_PHY_PORT_TO_CROSSBAR_PORT(ethctl->val);
    ...
    case ETHGETPHYPWR:
        phy_dev = enet_dev_get_phy_dev(dev, ethctl->phy_addr);
        if (!phy_dev)
            return -EFAULT;
    ...
    case ETHGETPHYEEE:
        ...
        if (!phy_dev)
            return -EFAULT;
        phy_dev_eee_get(phy_dev, &ethctl->ret_val);
        return copy_to_user(rq->ifr_data, ethctl, sizeof(struct ethctl_data)) ? -EFAULT : 0;
    case ETHGETPHYEEERESOLUTION:
        phy_dev = enet_dev_get_phy_dev(dev, ethctl->phy_addr);
        if (!phy_dev)
            return -EFAULT;
        phy_dev_eee_resolution_get(phy_dev, &ethctl->ret_val);
        return copy_to_user(rq->ifr_data, ethctl, sizeof(struct ethctl_data)) ? -EFAULT : 0;
    case ETHSETPHYEEEON:
...
static int tr_get_soft_sw_map(enetx_port_t *port, void *_ctx)
{
    ...
    if (port->dev &&  !(port->dev->priv_flags & IFF_HW_SWITCH))
...
#define PEAK_RATE 10000000000UL
    ...
    case ETHSWSOFTSWITCHING:    // turn on/off hw switching for a port
        ...
        else
        {
            ...
            if (!port->dev || port->n.port_netdev_role != PORT_NETDEV_ROLE_LAN || ethswctl->unit == 0)
                return -EOPNOTSUPP;
    ...
    case ETHSWPHYAUTONEGCAPADV:
        {
            ...
            caps &= ~(PHY_CAP_10_HALF | PHY_CAP_10_FULL | PHY_CAP_100_HALF | PHY_CAP_100_FULL | PHY_CAP_1000_HALF |
                PHY_CAP_1000_FULL | PHY_CAP_PAUSE | PHY_CAP_PAUSE_ASYM );
            ...
            if (ethswctl->autoneg_local & AN_FLOW_CONTROL_ASYM)
    ...
    case SIOCSCLEARMIBCNTR:
        {
            ...
            port_traverse_ports(port, tr_stats_clear, PORT_CLASS_PORT | PORT_CLASS_SW | PORT_CLASS_PORT_UNDEF, port);
            return 0;
        }
    case SIOCMIBINFO:
        {
    ...
    case SIOCSWANPORT:
        {
            ...
            port = ((enetx_netdev *)netdev_priv(dev))->port;
            return port_netdev_role_set(port, data ? PORT_NETDEV_ROLE_WAN : PORT_NETDEV_ROLE_LAN, 0/*is_bond*/) ? -EFAULT : 0;
        }
