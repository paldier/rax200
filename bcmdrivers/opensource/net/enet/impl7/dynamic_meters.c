...
{
    rdpa_cpu_reason_index_t reason_cfg_idx = {.dir = rdpa_dir_us, .reason = rdpa_cpu_rx_reason_ip_flow_miss};
    rdpa_cpu_reason_cfg_t reason_cfg = {};
    int rc;

    if ((rc = rdpa_cpu_reason_cfg_get(meter->cpu_obj, &reason_cfg_idx, &reason_cfg)))
    {
        printk("%s: failed to get CPU RX reason cfg; rc %d\n", __func__, rc);
        return;
    }
    reason_cfg.meter = meter->index;
    if ((rc = rdpa_cpu_reason_cfg_set(meter->cpu_obj, &reason_cfg_idx, &reason_cfg)))
    {
...
static void new_meter_set(dyn_meter_data_t *meter)
{
    ...
    rdpa_dir_index_t dir_idx = {.dir = rdpa_dir_us, .index = -1};
    int rc = BDMF_ERR_OK;
...
static int dyn_meter_poll_handler(void *arg)
{
    ...
    while (!kthread_should_stop())
    {
        ...
        if (queue_counters_get(meter, &dropped, &queued))
            continue;
