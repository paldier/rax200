...
inline void *_databuf_alloc(enet_ring_t *p_ring)
{
    ...
    /* NOTE: DO NOT cache invalidate the buffer at this stage, should be invalidated during packet
     * read from ring to prevent redundant invalidation cycles */
...
static int create_ring(int ring_id, uint32_t size, void **ring_base)
{
    ...
    *ring_base = (uint32_t*)(uintptr_t)phy_addr;
    ...
    enet_dbg("Creating Enet CPU ring for queue number %d with %d packets,Descriptor base=%pK, physical=0x%x\n ",ring_id,size,p_ring->base, phy_addr);
