...
enetx_port_t *port_create(port_info_t *port_info, enetx_port_t *parent_sw)
{
    ...
    if ((port_id == -1) && !port_info->is_undef)
    {
        ...
        enet_dbgv("skip port creation %d:%d\n", IS_ROOT_SW(parent_sw)?0:1, port_info->port);
        return NULL;
    }

    port = _port_create(port_type, port_id, parent_sw);

    if (port && port_info->is_undef)
        port->port_class = PORT_CLASS_PORT_UNDEF;

    // debug: overwrite port obj_name with unit:port for easier decode
    if (port && (port->port_class & (PORT_CLASS_PORT|PORT_CLASS_PORT_UNDEF)))
...
void sw_free(enetx_port_t **_p)
{
    ...
    if (p->port_class == PORT_CLASS_SW)
    {
        ...
        if (p->s.house_keeping_thread)
            kthread_stop(p->s.house_keeping_thread);
    }
...
int port_is_enet_link_handling(enetx_port_t *p)
{
    ...
    return (phy && (phy->phy_drv->phy_type != PHY_TYPE_PON) && 
...
//            (phy_is_crossbar(phy) && crossbar_group_external_endpoint_count(phy, NULL) > 1));
}
#endif
...
int port_init(enetx_port_t *port)
{
    ...
    if (port->p.phy)
    {
        ...
        if (port_is_enet_link_handling(port) &&
            !(port->p.port_cap == PORT_CAP_WAN_ONLY && port->n.port_netdev_role != PORT_NETDEV_ROLE_WAN))
...
int sw_init(enetx_port_t *sw)
{
    ...
    if (sw->s.ops->house_keeping_thread_fun)
        sw->s.house_keeping_thread = kthread_run(sw->s.ops->house_keeping_thread_fun, sw, "%sHouseKeeping", sw->obj_name);
...
init_ports:
    for (i = 0; i < sw->s.port_count; i++)
    {
        ...
        if (sw->s.ports[i]->port_class & PORT_CLASS_PORT_UNDEF)
        {
            enetx_port_t *p = sw->s.ports[i];
            // for undefined port, init associated mac_dev if exist 
            // but don't need to init port.
            if (p->p.mac) mac_dev_init(p->p.mac);
            if (p->p.port_cap == PORT_CAP_MGMT) mac_dev_enable(p->p.mac);
            continue;
        }
    ...
    {
        if ((!sw->s.ports[i]) ||
            (sw->s.ports[i]->port_class == PORT_CLASS_PORT_DETECT) ||
            (sw->s.ports[i]->p.child_sw == NULL))
            continue;

        rc = sw_init(sw->s.ports[i]->p.child_sw);
        if (rc && rc != EALREADY)
            break;
    }
...
int _port_traverse_ports(enetx_port_t *port, port_traverse_cb fn, port_class_t classes, void *ctx, int max_sw_depth)
{
    ...
    else if (port->port_class == PORT_CLASS_PORT_UNDEF)
    {
        if (classes & PORT_CLASS_PORT_UNDEF)
            return fn(port, ctx);
    }
    else
        return -1;
...
int port_netdev_role_set(enetx_port_t *self, port_netdev_role_t role, int is_bonding)
{
    ...
    if (!is_bonding)
    {   // role change thru bonding, device mac address is already set by bonding driver
    ...
    enet_dbg("SIOCSWANPORT:%s set as role %d\n", self->obj_name, role);
    return 0;
}
