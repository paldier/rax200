        ...
        case SIOCETHCTLOPS:
            ...
            switch(ethctl->op) {
                ...
                case ETHSETPHYPWROFF:
                    ...
                    if (swPort >= MAX_TOTAL_SWITCH_PORTS)
                    {
                        printk("ETHSETPHYPWROFF : Invalid swPort: %d \n", swPort);
                        val = -EINVAL;
                        break;
                    }
                ...
                case ETHGETPHYEEERESOLUTION:
                    BCM_ENET_DEBUG("op=%d phy_id=0x%x sub_port=%d\n",
                                    ethctl->op, ethctl->phy_addr, ethctl->val);
                    val = -EINVAL;
                    swPort = port_id_from_dev(dev);
                    if (swPort >= MAX_TOTAL_SWITCH_PORTS)
                    {
                    ...
                    if (ethctl->val != -1)
                        phy_id = enet_cb_port_to_phyid(
                                    LOGICAL_PORT_TO_UNIT_NUMBER(swPort),
                                    BP_PHY_PORT_TO_CROSSBAR_PORT(ethctl->val));
                    else
                        phy_id = enet_logport_to_phyid(swPort);
                    if (!phy_id)
                    {
                        printk("Invalid phy_id: %d, op=%d sub_port=%d\n",
                                    ethctl->phy_addr, ethctl->op, ethctl->val);
                        break;
                    }
