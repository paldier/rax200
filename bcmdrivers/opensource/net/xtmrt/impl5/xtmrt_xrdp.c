...
#define RDPA_XTM_CPU_TC_LOW  0
#define RDPA_XTM_CPU_TC_HIGH 1

/**** Externs ****/
extern bdmf_type_handle rdpa_xtmchannel_drv(void);

#define XTM_CPU_RX_QUEUES           2


/**** Globals ****/
int g_xtm_rxq_stats_received[XTM_CPU_RX_QUEUES]  = {}; /* for every queue */
int g_xtm_rxq_stats_dropped[XTM_CPU_RX_QUEUES]  = {};  /* for every queue */
...
static int xtm_ring_create_delete(int q_id, int size, rdpa_cpu_rxq_cfg_t *rxq_cfg);
...
int xtm_get_pkt_from_ring(int hw_q_id, FkBuff_t **ppFkb, rdpa_cpu_rx_info_t *info);


/**** Statistics ****/

static bdmf_object_handle rdpa_cpu_xtm_obj = NULL;
static int xtm_dump_rx = 0 ;
...
int bcmxapi_module_init(void)
{
   ...
   rdpa_cpu_index_set(cpu_xtm_attrs, RDPA_XTM_CPU_PORT) ;

   rdpa_cpu_num_queues_set(cpu_xtm_attrs, XTM_CPU_RX_QUEUES);

   if ((rc = bdmf_new_and_set(rdpa_cpu_drv(), NULL, cpu_xtm_attrs, &rdpa_cpu_xtm_obj)))
   {
      printk("%s:%s Failed to create cpu xtm%d object rc(%d)\n", __FILE__, __FUNCTION__, RDPA_XTM_CPU_PORT, rc);
      return -1;
   }
   ...
   rdpa_cpu_int_disable(RDPA_XTM_CPU_PORT, RDPA_XTM_CPU_LO_RX_QUEUE_ID);
   rdpa_cpu_int_clear(RDPA_XTM_CPU_PORT, RDPA_XTM_CPU_LO_RX_QUEUE_ID);
   ...
   rdpa_cpu_int_disable(RDPA_XTM_CPU_PORT, RDPA_XTM_CPU_HI_RX_QUEUE_ID);
   rdpa_cpu_int_clear(RDPA_XTM_CPU_PORT, RDPA_XTM_CPU_HI_RX_QUEUE_ID);
   ...
   for (reason = rdpa_cpu_reason_min; reason < rdpa_cpu_reason__num_of; reason++) {
      ...
      {
         tc = RDPA_XTM_CPU_TC_HIGH ;
         queue_id = RDPA_XTM_CPU_HI_RX_QUEUE_ID ;
      }
      else {
         tc = RDPA_XTM_CPU_TC_LOW ;
         queue_id = RDPA_XTM_CPU_LO_RX_QUEUE_ID ;
      }

      /* Reason to tc mapping is to be done once (currently done at ENET and in
      ** future, to be moved to common system
      **/
      //rc = rdpa_system_cpu_reason_to_tc_set(system_obj, reason, tc);
      rc = rc ? rc : rdpa_cpu_tc_to_rxq_set (rdpa_cpu_xtm_obj,tc, queue_id);
...
static int cfg_cpu_rx_queue(int queue_id, UINT32 queue_size, void *rx_isr)
{
    ...
    * back.
    */
   rdpa_cpu_rxq_cfg_get(rdpa_cpu_xtm_obj, queue_id, &rxq_cfg);
   rc = xtm_ring_create_delete(queue_id, queue_size, &rxq_cfg);
   if (rc)
      goto unlock_exit;
...
unlock_exit:
   if (!rc) 
      printk(KERN_NOTICE "Created XTM Rx queue %d with size %d\n", queue_id, (int)queue_size);
   else 
      printk(KERN_ERR "Failed to configure XTM CPU Rx queue %d with size %d \n", queue_id, (int) queue_size);
...
{
   int rc ;
   rc = rdpa_cpu_packet_get(RDPA_XTM_CPU_PORT, hw_q_id, info);
   if (!rc) {
      *ppFkb = fkb_init((uint8_t *)info->data , BCM_PKT_HEADROOM, (uint8_t *)(info->data + info->data_offset), info->size);
      ...
      if (unlikely(xtm_dump_rx))
         rdpa_cpu_rx_dump_packet("xtm_rx", RDPA_XTM_CPU_PORT, hw_q_id, info, 0);
   }
   return (rc) ;
}
...
UINT32 bcmxapi_rxtask(UINT32 ulBudget, UINT32 *pulMoreToDo)
{
   ...
   */
   do
   {
      ...
      rc = xtm_get_pkt_from_ring (RDPA_XTM_CPU_HI_RX_QUEUE_ID, &pFkb, &info);

      if (rc == BDMF_ERR_NO_MORE)
      {
          /* Receive packet from low priority cpu rx queue */
          rc = xtm_get_pkt_from_ring (RDPA_XTM_CPU_LO_RX_QUEUE_ID, &pFkb, &info);
          ...
          else
          {
              ...
              printk(KERN_ERR "Error in xtm_get_pkt_from_ring() qid %d rc (%d)\n", 
      ...
      {
         //printk ("xtm: packet. RD2=%x, len=%d\n", info.reason_data, info.size);
         bcmxtmrt_processRxPkt(pDevCtx, NULL, (UINT8 *)info.data,
                               (UINT16)info.reason_data, info.size);
         ulRxPktGood++;
         ulBudget--;
      }
      else  /* process cell */
...
static int runner_xtm_objects_init(void)
{
   ...
   rc = rdpa_cpu_get(RDPA_XTM_CPU_PORT, &cpu_obj);
