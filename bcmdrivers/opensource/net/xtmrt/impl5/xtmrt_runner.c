...
/**** Globals ****/
int g_xtm_rxq_stats_received[2]  = {}; /* for every queue */
int g_xtm_rxq_stats_dropped[2]  = {};  /* for every queue */
...
static struct proc_dir_entry *xtm_dir;
static bdmf_object_handle rdpa_cpu_obj = NULL;
RING_DESCRIPTOR_S xtm_ring[2]  = {};
...
int bcmxapi_module_init(void)
{
   ...
   while (!rdpa_cpu_reason_cfg_get_next(rdpa_cpu_obj, &reason_cfg))
   {
      ...
      if ((reason_cfg.reason == rdpa_cpu_rx_reason_oam) ||
          ...
          (reason_cfg.reason == rdpa_cpu_rx_reason_direct_flow ||
           reason_cfg.reason == rdpa_cpu_rx_reason_ipsec))
      {
         continue;
      }
      ...
      if (reason_cfg.reason == rdpa_cpu_rx_reason_etype_pppoe_d ||
          ...
          reason_cfg.reason == rdpa_cpu_rx_reason_etype_802_1ag_cfm ||
          ...
          reason_cfg.reason == rdpa_cpu_rx_reason_udef_0 ||
#endif
...
/*---------------------------------------------------------------------------
 * int bcmxapi_rxq_queued_get(void)
 * Description:
 ...
 *---------------------------------------------------------------------------
 */
static inline int bcmxapi_rxq_queued_get(int qidx)
{
    CPU_RX_DESCRIPTOR *p;
    int count;
 ...
 *---------------------------------------------------------------------------
 */
static void bcmxapi_rxq_stat_cb(int qid, extern_rxq_stat_t *stat, bdmf_boolean clear)
{
    ...
    stat->queued  = bcmxapi_rxq_queued_get(queue_idx);
...
static int cfg_cpu_rx_queue(int queue_id, UINT32 queue_size, void *rx_isr)
{
   ...
   if (rc == 0)
   {
      ...
      rc = rdpa_cpu_rx_create_ring(&xtm_ring[queue_id - RDPA_XTM_CPU_RX_QUEUE_ID_BASE],queue_size,&ring_base);
      if (rc < 0)
         goto unlock_exit;
      rxq_cfg.ring_head = ring_base;
      rxq_cfg.rxq_stat = bcmxapi_rxq_stat_cb;
      rxq_cfg.ic_cfg.ic_enable = false;
      rc = rdpa_cpu_rxq_cfg_set(rdpa_cpu_obj, queue_id, &rxq_cfg);
   }
unlock_exit:
   if (!rc) 
   {
      printk(KERN_NOTICE "Created XTM Rx queue %d with size %d,Descriptor base=%pK\n", 
             queue_id, (int)queue_size, ring_base);
   }   
   return rc;
...
{
   if (rdpa_cpu_ring_not_empty(xtm_ring[XTM_RDPA_CPU_HI_RX_QUEUE_IDX].head) ||
       rdpa_cpu_ring_not_empty(xtm_ring[XTM_RDPA_CPU_LO_RX_QUEUE_IDX].head))
   {
       return true;
   }
   return false;
...
UINT32 bcmxapi_rxtask(UINT32 ulBudget, UINT32 *pulMoreToDo)
{
   ...
   */
   do
   {
      ...
      rc = rdpa_cpu_get_pkt_from_ring(&xtm_ring[XTM_RDPA_CPU_HI_RX_QUEUE_IDX], 
                                      &pFkb, &info);
      if (rc == BDMF_ERR_NO_MORE)
      {
          /* Receive packet from low priority cpu rx queue */
          rc = rdpa_cpu_get_pkt_from_ring(&xtm_ring[XTM_RDPA_CPU_LO_RX_QUEUE_IDX],
                                          &pFkb, &info);
          if (rc == BDMF_ERR_NO_MORE)
          {
          ...
          else
          {
              ...
              printk(KERN_NOTICE "Error in rdpa_cpu_get_pkt_from_ring() qid %d rc (%d)\n", 
