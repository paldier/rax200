...
void fpm_sync_buffer_for_cpu(u32 token, u32 head, u32 tail, u32 flags)
{
	 ...
	 * in nbuff.h */
	paddr = fpm->pool_pbase[FPM_TOKEN_POOL(token)] +
		((uintptr_t)start -
		 (uintptr_t)fpm->pool_vbase[FPM_TOKEN_POOL(token)]);
...
static int fpm_resource_alloc(struct platform_device *pdev)
{
	...
	if (BcmMemReserveGetByName(BUFFER_MEMORY_BASE_ADDR_STR, &pool0_vbase,
				   &pool0_size))
		return -EFAULT;
	fdev->pool_vbase[0]  = (u32 *)pool0_vbase;
	fdev->pool_pbase[0] = (phys_addr_t)virt_to_phys(pool0_vbase);
...
static int fpm_pool_size_check_adjust(struct platform_device *pdev)
{
	...
	for (pool_idx = 1; pool_idx < FPM_POOL_NUM; pool_idx++) {
		...
		fdev->pool_vbase[pool_idx] = (u32 *)((uintptr_t)fdev->pool_vbase[0] +
					      (pool_size * pool_idx));
		fdev->pool_pbase[pool_idx] = (phys_addr_t)virt_to_phys(
						fdev->pool_vbase[pool_idx]);
	}
