...
static int add_ic(rdpactl_classification_rule_t *rule, bdmf_object_handle *ingress_class_obj)
{
    ...
    cfg.generic_filter_location = (rdpa_filter_location_t)rule->generic_filter_location;
...
add_class_exit:
    if (ret) 
        CMD_IC_LOG_ERROR("rdpa_ic_add_class OUT FAILED: field(0x%x) rc(%d)", pRule->field_mask, rc);
...
del_class_exit:
    if (ret) 
        CMD_IC_LOG_ERROR("rdpa_cmd_ic_ioctl() OUT: FAILED: field(0x%x) rc(%d)", pRule->field_mask, rc);
    bdmf_unlock();
    return ret;
}
...
add_flow_exit:
    if (ret) 
        CMD_IC_LOG_ERROR("rdpa_ic_add_flow OUT: FAILED: field(0x%x) rc(%d)", pRule->field_mask, rc);
...
del_flow_exit:
    if (ret) 
        CMD_IC_LOG_ERROR("rdpa_ic_del_flow OUT: FAILED: field(0x%x) rc(%d)", pRule->field_mask, rc);
    ...
    case RDPA_IOCTL_DS_WAN_UDP_FILTER_CMD_ADD:
        {
            ...
            rule.opbit_remark = -1;
            rule.ipbit_remark = -1;
            rule.type = RDPACTL_IC_TYPE_GENERIC_FILTER;
            rule.generic_filter_location = RDPACTL_ALL_TRAFFIC;
            rule.dir = rdpactl_dir_ds;
            rule.action = RDPACTL_IC_ACTION_DISCARD;
    ...
    for (field_idx = 0; field_idx < IQ_KEY_FIELD_MAX; field_idx++)
    {
        if (iq_param->key_mask & (0x1 << field_idx))
        {
            ...
            case IQ_KEY_FIELD_L3_PROTO:
                rdpactl_rule->field_mask |= RDPACTL_IC_MASK_L3_PROTOCOL;
                if (key_data->ip_proto == ETH_P_IP)
                    rdpactl_rule->version = 1;
                else if (key_data->ip_proto == ETH_P_IPV6)
                    rdpactl_rule->version = 2;
                else
                    rdpactl_rule->version = 0;
                break;
            case IQ_KEY_FIELD_IP_PROTO:
                rdpactl_rule->field_mask |= RDPACTL_IC_MASK_IP_PROTOCOL;
                rdpactl_rule->protocol = key_data->ip_proto;
                break;
            case IQ_KEY_FIELD_SRC_IP:
                rdpactl_rule->field_mask |= RDPACTL_IC_MASK_SRC_IP;
                if (key_data->is_ipv6) {
                        /* IPv6 address is in host byte order, but RDPA requires
                         * network byte order */
                        rdpactl_rule->ip_family |= RDPACTL_IP_FAMILY_IPV6;
                        ...
                        rdpactl_rule->ip_family |= RDPACTL_IP_FAMILY_IPV4;
                        rdpactl_rule->src_ip.ipv4 = key_data->src_ip[0];
                }
                break;
            case IQ_KEY_FIELD_DST_IP:
                rdpactl_rule->field_mask |= RDPACTL_IC_MASK_DST_IP;
                if (key_data->is_ipv6) {
                        /* IPv6 address is in host byte order, but RDPA requires
                         * network byte order */
                        rdpactl_rule->ip_family |= RDPACTL_IP_FAMILY_IPV6;
...
int rdpa_iq_ingress_class_add(void *iq_param)
{
    ...
    rule.type = RDPACTL_IC_TYPE_GENERIC_FILTER;
    rule.dir = rdpactl_dir_ds;
    rule.generic_filter_location = RDPACTL_FLOW_MISSED_TRAFFIC;

    if (action->type == IQ_ACTION_TYPE_PRIO)
    {
        rule.action = RDPACTL_IC_ACTION_HOST;
