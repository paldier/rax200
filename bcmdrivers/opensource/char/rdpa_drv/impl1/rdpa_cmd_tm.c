...
{
#if defined(CONFIG_BCM_PON) || defined(CONFIG_BCM963158) /* TBD - Understand why different */
    queue_st *pq;
    int ret;
    ...
    rdpa_drv_ioctl_tm_t *ptm)
{
    ...
    CMD_TM_LOG_DEBUG("rdpa_egress_tm_queue_cfg_set(): arbiter(%u) tm(%u) q_index(%u) q_index2(%u) q_id(%u) drop_alg(%u) qsize(%u) weight(%u) shaper_rate(%u) clean(%u)",
                     arbiter_mode, ptm->tm_id, ptm->index, q_index, ptm->q_id, queue_cfg.drop_alg, ptm->qsize, ptm->weight, ptm->shaping_rate, ptm->q_clean);
...
int rdpa_cmd_tm_ioctl(unsigned long arg)
{
    ...
    case RDPA_IOCTL_TM_CMD_GET_BEST_EFFORT_TM_ID:
    {
        ...
        rdpa_tm_queue_cfg_t queue_cfg = {};
        bdmf_object_handle sched = NULL;
        uint32_t q_idx = INVALID_ID;
        int i;

        tm.tm_id = INVALID_ID;

        for (i = 0; i < MAX_Q_PER_TM; i++) {
            ret = rdpa_cmd_drv_get_tm_key_q_idx(tm.dev_id, i, tm.dir, &tm_key, &q_idx);
            if (ret == RDPA_DRV_ERROR)
                goto ioctl_exit;

            if (ret == RDPA_DRV_SUCCESS && q_idx != INVALID_ID) {
                rc = rdpa_egress_tm_get(&tm_key, &sched);
                if (rc) {
                    CMD_TM_LOG_DEBUG("rdpa_egress_tm_get() failed: tm(%d) rc(%d)", (int)tm_key.index, rc);
                    continue;
                }

                rc = rdpa_egress_tm_queue_cfg_get(sched, 0, &queue_cfg);
                bdmf_put(sched);
                if (rc) {
                    CMD_TM_LOG_DEBUG("rdpa_egress_tm_queue_cfg_get() failed: rc(%d)", rc);
                    continue;
                }

                if (queue_cfg.best_effort) {
                    tm.tm_id = tm_key.index;
                    tm.dir   = tm_key.dir;
                    break;
                }
            }
