                                  ...
                                  int pkt_type)
{
   ...
   hlist_for_each_entry(dst, head, hlist)
   {
      ...
      if (is_routed)
      {
         /* is_routed flag will be true for both host generated and WAN
            forwarded packets since the src_dev == pif->dev(i.e. br0) for
            both scenarios. However, for Host generated mcast packet, the
            skb->pkt_type is set to PACKET_HOST. Forward such packets as
            long as a snooping entry exists for the LAN device regardless
            the snoop entry type is BCM_MCAST_IF_ROUTED or
            BCM_MCAST_IF_BRIDGED */
         if (dst->type != BCM_MCAST_IF_ROUTED && pkt_type != PACKET_HOST)
         {
            continue;
         }
      }
...
int bcm_mcast_igmp_display(struct seq_file *seq, bcm_mcast_ifdata *pif)
{
   ...
   seq_printf(seq, "Bridge %s igmp snooping %d  lan2lan-snooping %d/%d, rate-limit %dpps, priority %d\n",
              pif->dev->name,
              pif->igmp_snooping, 
