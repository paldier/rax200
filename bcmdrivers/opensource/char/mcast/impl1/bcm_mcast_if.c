...
static void bcm_mcast_if_add_lower_port_to_if(struct net_device *lowerDev, bcm_mcast_ifdata* pif)
{
   bcm_mcast_lower_port* lowerPort = kmalloc(sizeof(bcm_mcast_lower_port), GFP_ATOMIC);
   lowerPort->port_ifindex = lowerDev->ifindex;
#if defined(CONFIG_BR_IGMP_SNOOP)
   lowerPort->querying_port = 0;
...
static bcm_mcast_ifdata *bcm_mcast_if_create(int ifindex)
{
   bcm_mcast_ifdata  *pif;
   ...
   {
      bcm_mcast_if_add_lower_port_to_if (lowerdev, pif);
   }
...
static void bcm_mcast_if_handle_lower_up(struct net_device *dev)
{
   ...
   if (pif)
   {
      ...
      spin_lock_bh(&pif->config_lock);
      lowerPort = bcm_mcast_if_get_lower_port_by_ifindex(pif, dev->ifindex);
      /* Only add the powerPort if it doesn't exist yet */
      if (NULL == lowerPort)
      {
   ...
   switch (event)
   {
      ...
      case NETDEV_UP:
         if ( dev->priv_flags & IFF_EBRIDGE )
         {
            ...
            if ( NULL == pif )
            {
               bcm_mcast_if_create(dev->ifindex);
