...
int ivi_v4v6_xmit(struct sk_buff *skb, unsigned int mtu) {
	...
	if (ip4h->protocol == IPPROTO_ICMP) {
		if ((ip4h->frag_off & htons(IP_MF)) || (ip4h->frag_off & htons(IP_OFFSET))) {
			...
			*/
			local_bh_disable();
			err = ip_defrag(skb, IP_DEFRAG_MAPT);
			local_bh_enable();
	...
	if (ip4h->frag_off & htons(IP_DF))
	{
		if (plen + sizeof(struct ipv6hdr) > mtu)
		{
			...
			//MTU for LAN = WAN MTU - extra 20 in ipv6 header - frag header 8 byte
			dst_mtu = (u32)mtu - 20 - 8;
	...
	*(__u32 *)ip6h = __constant_htonl(0x60000000);
	...
	if ((frag_mode == 2 && ip4h->frag_off) || frag_mode == 3) {
		u16 j, k, m, A;
	...
	if (transport == MAP_E) {
		...
		if ((frag_mode == 1 || frag_mode == 3) && fh)
		{
			...
			newskb->mapt_forward = MAPT_FORWARD_MODE1;
		}
	}
	...
	if (frag_mode == 2)
	{
		newskb->mapt_forward = MAPT_FORWARD_MODE2;

		if (ip4h->frag_off) {
			newskb->mapt_offset = ((ntohs(ip4h->frag_off) & IP_OFFSET) << 3) & IP6_OFFSET;
			newskb->mapt_id = htonl(newp);
		}
		if (ip4h->frag_off & htons(IP_MF))
			newskb->mapt_mf = 1;
	}
