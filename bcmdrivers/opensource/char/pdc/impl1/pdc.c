	      ...
	      u32 buf_len, u32 flags)
{
	...
	dev_dbg(dev,
		"Writing rx descriptor for ch %u at index %u with length %04u. flags %#x, %llx\n",
	      ...
	      u32 flags)
{
	...
	dev_dbg(dev,
		"Writing tx descriptor for ch %u at index %u with length %04u, flags %#x, %llx\n",
...
pdc_crypto_engine_poll(struct pdc_state *pdcs)
{
	...
	spin_lock_irqsave(&pdcs->pdc_lock, flags);
	...
	spin_unlock_irqrestore(&pdcs->pdc_lock, flags);
	return msg;
}
...
int pdc_channel_init(struct platform_device *pdev, int chan)
{
	...
	dev_dbg(&pdev->dev, "DMA rings allocation size %lu", PDC_RING_SIZE);
