                       ...
                       unsigned int command, unsigned long arg )
{
    ...
    case BOARD_IOCTL_FLASH_WRITE:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            ...
            case BCM_IMAGE_WHOLE:
                ...
                if(ret == 0 && noReboot == 0)
                {
                    /*resetPwrmgmtDdrMips();*/ /* foxconn removed */
                }
                else
                {
                    if (ret != 0)
    ...
    case BOARD_IOCTL_FLASH_READ:
        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0) {
            ...
            case SET_BOARDID: {
                    /* foxconn, cliff wang@new wau to set board id */
            ...
            case GET_BOARDID: {
                    /* foxconn, cliff wang@new wau to get board id */
                    NVRAM_DATA *pNvramData;
                    pNvramData = readNvramData();
                    if (pNvramData == NULL)
                         return -ENOMEM;
    ...
    {
        NVRAM_DATA *pNvramData;
        PMAC_INFO g_pMacInfo;

        if (copy_from_user((void*)&ctrlParms, (void*)arg, sizeof(ctrlParms)) == 0)
        {
            if ((NULL != (pNvramData = readNvramData()))&&(NULL != (g_pMacInfo = get_mac_info())))
            {
