...
void __init kerSysEarlyFlashInit( void )
{
    ...
    if( bcm_get_root_propdata(BRCM_NVRAM_PROP, (char*)&inMemNvramData, sizeof(NVRAM_DATA)) == 0 )
    {
        ...
        memcpy(cfe_boot, BOOTLOADER_CFE_STR, sizeof(cfe_boot));
...
#define NVRAM_FILE_NAME         "/data/.kernel_nvram.setting"   // foxconn added
#else
#define NVRAM_FILE_NAME         "/data/nvram"   // foxconn added
#endif
#define NVS_FILE_NAME           "/data/dectnvs" // foxconn added
#define SP_FILE_NAME            "/storage2/sp"      // foxconn added
#define BD_FILE_NAME            "/data/board_data"      // foxconn added
#define DPF_FILE_NAME           "/data/dpf"      // foxconn added
#define BD_FILE_NAME2           "/storage1/board_data"  // foxconn added
/* Foxconn added end */
...
//  0 - ok
//  -1 - fail
int kerSysFoxconnNvramSet(char *buf, int len)
{
    if( bootFromNand )
    {
        int ret = -1;
        ...
        fp = filp_open(NVRAM_FILE_NAME, O_RDWR | O_TRUNC | O_CREAT,  S_IRUSR | S_IWUSR);
        if (!IS_ERR(fp))
        {
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 0, 00)
            if (fp->f_op && fp->f_op->write)
...
int kerSysFoxconnNvramGet(char *string, int strLen, int offset)
{
    if( bootFromNand )
    {
        struct file *fp;
        ...
        fp = filp_open(NVRAM_FILE_NAME, O_RDONLY, 0);
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 0, 0)
        if (!IS_ERR(fp) && fp->f_op && fp->f_op->read)
#else
        if (!IS_ERR(fp))
...
int kerSysNvDataSet(char *string, int strLen, int offset)
{
    ...
    if( bootFromNand )
    {
        ...
        fp = filp_open(NVS_FILE_NAME, O_RDWR | O_TRUNC | O_CREAT,
            S_IRUSR | S_IWUSR);
        ...
        if (!IS_ERR(fp) && fp->f_op && fp->f_op->write)
        {
            ...
            if((int) fp->f_op->write(fp, (void *) string, strLen, &fp->f_pos) != strLen)
                printk("Failed to write nvram to '%s'.\n", NVS_FILE_NAME);
...
int kerSysNvDataGet(char *string, int strLen, int offset)
{
    ...
    if( bootFromNand )
    {
        ...
        memset(string, 0x00, strLen);
        fp = filp_open(NVS_FILE_NAME, O_RDONLY, 0);
        if (!IS_ERR(fp) && fp->f_op && fp->f_op->read)
        {
            fs = get_fs();
            set_fs(get_ds());
            ...
            if((len = (int) fp->f_op->read(fp, (void *) string, strLen, &fp->f_pos)) <= 0)
                printk("Failed to read psi from '%s'\n", NVS_FILE_NAME);
...
int kerSysSPSet(char *string, int strLen, int offset)
{
    ...
    if( bootFromNand )
    {
        ...
        fp = filp_open(SP_FILE_NAME, O_RDWR | O_TRUNC | O_CREAT,
            S_IRUSR | S_IWUSR);
...
int kerSysSPGet(char *string, int strLen, int offset)
{
    ...
    if( bootFromNand )
    {
        ...
        memset(string, 0x00, strLen);
        fp = filp_open(SP_FILE_NAME, O_RDONLY, 0);
        if (!IS_ERR(fp) && fp->f_op && fp->f_op->read)
        {
            fs = get_fs();
            set_fs(get_ds());
...
int kerSysBDSet(char *buf, int len)
{
    if( bootFromNand )
    {
	    int ret = -1;
        ...
        fp = filp_open(BD_FILE_NAME2, O_RDWR | O_TRUNC | O_CREAT,  S_IRUSR | S_IWUSR);
        if (!IS_ERR(fp))
        {
            if (fp->f_op && fp->f_op->write)
            {
...
int kerSysBDGet(char *string, int strLen, int offset)
{
    ...
    if( bootFromNand )
    {
        ...
        memset(string, 0x00, strLen);
        fp = filp_open(BD_FILE_NAME2, O_RDONLY, 0);
        
        if ( IS_ERR(fp) )
            fp = filp_open(BD_FILE_NAME, O_RDONLY, 0);  /* it's for backward compatible. */
...
int kerSysDPFSet(char *string, int strLen, int offset)
{
    ...
    if( bootFromNand )
    {
        ...
        fp = filp_open(DPF_FILE_NAME, O_RDWR | O_TRUNC | O_CREAT,
            S_IRUSR | S_IWUSR);
...
int kerSysDPFGet(char *string, int strLen, int offset)
{
    ...
    if( bootFromNand )
    {
        ...
        memset(string, 0x00, strLen);
        fp = filp_open(DPF_FILE_NAME, O_RDONLY, 0);
        if (!IS_ERR(fp) && fp->f_op && fp->f_op->read)
        {
            fs = get_fs();
            set_fs(get_ds());
