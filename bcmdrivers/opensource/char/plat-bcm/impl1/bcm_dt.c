...
#if 1
static int __init dt_get_buffer_prop(unsigned long node, uint64_t* base, uint64_t* size)
{
    const __be32 *endp;
    const __be32 *reg;
    int regsize;
...
#if 1
            if ( strcmp(dt_scan_mem_str[i], BUFFER_MEMORY_BASE_ADDR_STR) == 0 ) {
                ret = dt_get_buffer_prop(node, &base, &size);
            } else
#endif
            {
                ret = dt_get_memory_prop(node, &base, &size);
            }
