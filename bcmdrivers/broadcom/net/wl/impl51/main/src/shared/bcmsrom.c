 ...
 *
 * $Id: bcmsrom.c 766705 2018-08-10 03:13:01Z $
 */
...
/* srom version check: Current valid versions are: 1-5, 8-11, 12, 13, 15, 16, 17, 18 SROM_MAXREV
 * This is a bit mask of all valid SROM versions.
 */
#define VALID_SROMREV   0x7bf3e
...
#if defined(BCMUSBDEV)
static int get_si_pcmcia_srom(si_t *sih, osl_t *osh, volatile uint8 *pcmregs,
                              uint boff, uint16 *srom, uint bsz, bool check_crc);
#if defined(WLTEST) || defined(DHD_SPROM)
static int set_si_pcmcia_srom(si_t *sih, osl_t *osh, volatile uint8 *pcmregs,
...
/**
 * The contents of this array is a first attempt, is likely incorrect for 4365/4366, needs to be
 * edited in a later stage.
 */
static char BCMATTACHDATA(defaultsromvars_4366)[] =
	"sromrev=13\0"
	"subvid=0x106b\0"
	"vendid=0x14e4\0"
	"devid=0x43c3\0"
...
static char BCMATTACHDATA(defaultsromvars_4347)[] =
	"cckdigfilttype=4\0"
	"sromrev=11\0"
	"boardrev=0x1102\0"
	"boardtype=0x0771\0"
           ...
           uint byteoff, uint nbytes, uint16 *buf)
{
	...
	if (old[0] == 0xffff) {
		...
		if (buf[SROM11_SIGN] == SROM11_SIGNATURE) {
			...
			/* nbytes can be < SROM18 bytes since host limits trasnfer chunk size
			 * to 1518 Bytes
			 */
...
static int
	srom_cmd_si_pcmcia(osl_t *osh, volatile uint8 *pcmregs, uint8 cmd)
{
	uint8 status = 0;
	uint wait_cnt = 0;
...
static int
srom_read_si_pcmcia(osl_t *osh, volatile uint8 *pcmregs, uint16 addr, uint16 *data)
{
...
static int
srom_write_si_pcmcia(osl_t *osh, volatile uint8 *pcmregs, uint16 addr, uint16 data)
{
	uint8 addr_l, addr_h, data_l, data_h;
	int rc;
 ...
 */
static int
get_si_pcmcia_srom(si_t *sih, osl_t *osh, volatile uint8 *pcmregs,
                   uint boff, uint16 *srom, uint bsz, bool check_crc)
{
	uint i, nw, woff, wsz;
	int err = 0;
 ...
 */
static int
set_si_pcmcia_srom(si_t *sih, osl_t *osh, volatile uint8 *pcmregs,
                   uint boff, uint16 *srom, uint bsz)
{
	uint i, nw, woff, wsz;
	uint16 word;
...
BCMATTACHFN(get_max_cis_size)(si_t *sih)
{
	...
	if (sih) {
		if (sih->ccrev >= 66)
			...
			max_cis_size = CIS_SIZE_SW_1180B;
		else if (sih->ccrev >= 49)
			max_cis_size = CIS_SIZE_12K;
	}
...
BCMATTACHFN(initvars_srom_pci)(si_t *sih, void *curmap, char **vars, uint *count)
{
	...
	if (si_is_sprom_available(sih)) {
		if (ISSIM_ENAB(sih) && BCM43684_CHIP(sih->chip)) {
			...
			BS_ERROR(("skip srom read in 43684 veloce development\n"));
			err = 1;
		} else {
...
#ifdef NVSRCX
	if (sromrev != 0)
		nvram_append((void *)sih, *vars, *count);
#endif // endif
#if defined(BCMPCIEDEV_SROM_FORMAT) && defined(WLC_TXCAL)
...
BCMATTACHFN(initvars_cis_pcmcia)(si_t *sih, osl_t *osh, char **vars, uint *count)
{
	...
	/* freed in same function */
	if ((cis = MALLOC_NOPERSIST(osh, data_sz)) == NULL)
		return (-2);
...
#else
	size = (SROM16_SIGN + 1) * 2;	/* must big enough for SROM16 */
#endif // endif
	return size;
}
...
BCMATTACHFN(initvars_cis_usbdriver)(si_t *sih, osl_t *osh, char **vars, uint *count)
{
	...
	/* freed in same function */
	if ((cis = MALLOC_NOPERSIST(osh, OTP_SZ_MAX)) == NULL) {
		return -1;
	}
...
BCMATTACHFN(initvars_srom_si_usbdriver)(si_t *sih, osl_t *osh, char **vars, uint *varsz)
{
	...
	if ((_varsz != 0) && (_vars != NULL)) {
		...
		/* freed in same function */
		base = MALLOC_NOPERSIST(osh, len + 2); /* plus 2 terminating \0 */
		if (base == NULL) {
		...
		bcopy((const void *)vstr_end, (base + len), strlen(vstr_end));
		...
		*varsz = len;
		...
		/* freed in same function */
		base = MALLOC_NOPERSIST(osh, (len + 1));
		if (base == NULL) {
			BS_ERROR(("initvars_srom_si: MALLOC failed.\n"));
			return BCME_ERROR;
		}
...
static uint8 BCMATTACHDATA(defcis4364)[] = { 0x20, 0x4, 0xd0, 0x2, 0x64, 0x43, 0xff, 0xff };
...
BCMATTACHFN(initvars_srom_si)(si_t *sih, osl_t *osh, void *curmap, char **vars, uint *varsz)
{
	...
	case BCM43018_CHIP_ID: ciss = 1; defcis = defcis43018; hdrsz = 4; break;
	case BCM4364_CHIP_ID: ciss = 1; defcis = defcis4364; hdrsz = 4; break;
	case BCM4369_CHIP_GRPID: ciss = 1; defcis = defcis4369; hdrsz = 4; break;
	case BCM4347_CHIP_GRPID: ciss = 1; defcis = defcis4347; hdrsz = 4; break;
	case BCM4350_CHIP_ID:
	case BCM4354_CHIP_ID:
	case BCM43556_CHIP_ID:
	case BCM43558_CHIP_ID:
