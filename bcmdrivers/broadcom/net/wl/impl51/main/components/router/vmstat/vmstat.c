/* Modified by Broadcom Corp. Portions Copyright (c) Broadcom Corp, 2010. */
...
#define ITLBSTAT      0x00000020
#define DTLBSTAT      0x00000040
#define CPUCACHESTAT  0x00000080
...
static void new_header(void){
  ...
  if (statMode & ITLBSTAT) {
    printf(" -----------itlb------------");
  } else if(statMode & DTLBSTAT) {
    printf(" -------dtlb--------");
  } else if (statMode & CPUCACHESTAT) {
    printf(" ------------i/d cache-------------");
  }
  printf("\n");
  ...
  if (statMode & ITLBSTAT) {
    printf(" %6s %6s %6s", "ia", "im", "jm");
  } else if(statMode & DTLBSTAT) {
    printf(" %6s %6s", "ja", "jm");
  } else if (statMode & CPUCACHESTAT) {
    printf(" %6s %6s %6s %6s", "ia", "im", "dw", "dm");
  }
...
static void new_format(int mode) {
  ...
  unsigned long itlb_access[2], itlb_miss[2], ijtlb_miss[2];
  unsigned long itlba, itlbm, ijtlbm;
  unsigned long djtlb_access[2], djtlb_miss[2];
  unsigned long djtlba, djtlbm;
  unsigned long icache_access[2], icache_miss[2], dcache_wb[2], dcache_miss[2];
  unsigned long icachea, icachem, dcachewb, dcachem;
  ...
  if (mode & ITLBSTAT) {
	const char format[] = " %6lu %6lu %6lu";

	getitlbstat(itlb_access, itlb_miss, ijtlb_miss);

	printf(format, *itlb_access, *itlb_miss, *ijtlb_miss);
  } else if (mode & DTLBSTAT) {
	const char format[] = " %6lu %6lu";

	getdjtlbstat(djtlb_access, djtlb_miss);

	printf(format, *djtlb_access, *djtlb_miss);
  } else if (mode & CPUCACHESTAT) {
	const char format[] = " %6lu %6lu %6lu %6lu";

	getcpucachestat(icache_access, icache_miss, dcache_wb, dcache_miss);

	printf(format, *icache_access, *icache_miss, *dcache_wb, *dcache_miss);
  }
  ...
  for(i=1;i<num_updates;i++) { /* \\\\\\\\\\\\\\\\\\\\ main loop ////////////////// */
	...
	getitlbstat(itlb_access+tog, itlb_miss+tog, ijtlb_miss+tog);

	itlba = itlb_access[tog]-itlb_access[!tog];
	itlbm = itlb_miss[tog]-itlb_miss[!tog];
	ijtlbm = ijtlb_miss[tog]-ijtlb_miss[!tog];
	printf(format, itlba, itlbm, ijtlbm);

    } else if (mode & DTLBSTAT) {
	const char format[] = " %6lu %6lu";

	getdjtlbstat(djtlb_access+tog, djtlb_miss+tog);

	djtlba = djtlb_access[tog]-djtlb_access[!tog];
	djtlbm = djtlb_miss[tog]-djtlb_miss[!tog];
	printf(format, djtlba, djtlbm);

    } else if (mode & CPUCACHESTAT) {
	const char format[] = " %6lu %6lu %6lu %6lu";

	getcpucachestat(icache_access+tog, icache_miss+tog, dcache_wb+tog, dcache_miss+tog);

	icachea = icache_access[tog]-icache_access[!tog];
	icachem = icache_miss[tog]-icache_miss[!tog];
	dcachewb = dcache_wb[tog]-dcache_wb[!tog];
	dcachem = dcache_miss[tog]-dcache_miss[!tog];
	printf(format, icachea, icachem, dcachewb, dcachem);
    }
    printf("\n");
  }
}
...
int main(int argc, char *argv[]) {
  ...
  perfopt = (statMode & (ITLBSTAT | DTLBSTAT | CPUCACHESTAT));
  if (perfopt & (perfopt - 1)) {
    printf("May use only one among I/P/C options\n");
    exit(0);
  }
  ...
  switch(statMode){
	...
	case(ITLBSTAT):
	case(DTLBSTAT):
	case(CPUCACHESTAT):  new_format(statMode);
			     break;
