 ...
 * $Id: cgi_wan.c 758922 2018-04-23 03:19:48Z $
 */
...
static int
ej_bond_wan_display(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	websWrite(wp, "onMouseOver=\"return overlib(\'Enables the use of the Bonding/"
			...
			"WAN Bonding ports are %s.\', LEFT);\"\n",
			nvram_match("bond_wan_ifnames", "") ? "eth0 eth4" : nvram_safe_get("bond_wan_ifnames"));
	websWrite(wp, "onMouseOut=\"return nd();\">\n");
	websWrite(wp, "Bonding:&nbsp;&nbsp;\n");
	websWrite(wp, "</th>\n");
	websWrite(wp, "<td>&nbsp;&nbsp;</td>\n");
	websWrite(wp, "<td>\n");
	websWrite(wp, "<select name=\"bond_wan\">\n");
	websWrite(wp, "<option value=\"1\" %s>Enabled</option>",
			nvram_match("bond_wan", "1") ? "selected": "\n");
	websWrite(wp, "<option value=\"0\" %s>Disabled</option>",
			nvram_match("bond_wan", "0") ? "selected": "\n");
	websWrite(wp, "</select>\n");
	websWrite(wp, "</td>\n");
	websWrite(wp, "</tr>\n");
	websWrite(wp, "</table>\n");
...
REG_EJ_HANDLER(bond_wan_display);
