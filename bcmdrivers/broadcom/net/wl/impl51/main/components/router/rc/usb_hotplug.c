 ...
 *
 * $Id: usb_hotplug.c 741209 2018-01-16 08:14:45Z $
 */
...
int wrapped_mount(char *source, char *target)
{
    ...
    struct stat fs;
    int wait_count = 0, retry_count = 0;
     ...
     */
    retry_count = atoi(acosNvramConfig_get("usb_mount_retry"));
    if (retry_count == 0)
        retry_count = 120; //Wait for nvram load default value
...
#endif
        if (++wait_count < retry_count) /* wait for 20 seconds at most */
            goto retry;
    } 

    /* If usb mount target is /dev/sdx, should sleep for a while
       Some device only have /dev/sdx be mounted
       Some device report partitions (/dev/sdx1~15), /dev/sdx should not be mounted at the same time
       Otherwise, it may make partitions be mounted fail due to resource is busy */
    if (strlen(source) == 8) // /dev/sdx
        sleep(3);
...
#if (defined WNDR3400v2) || (defined R6300v2) || (defined R7000) || defined(R8000) || defined(BCA_HNDROUTER)
...
#if 0   /* Remove to use Paragon HFS plus module */
    else if ((rval = mount(source, target, "hfsplus", 0, "")) == 0)
    {
        // Mount hfsplus journaled disk with r/w mode
        memset(buf, 0x0, sizeof(buf));
        snprintf(buf, sizeof(buf), "mount -t hfsplus -o remount,force,rw %s", target);
        system(buf);
    ...
    for(i=0; i<MAX_USB_DEV_NUM; i++)
    {
    	if(curUsbDev[i].majorNum==0 && curUsbDev[i].minorNum==0)
    	{
    		...
    		if (fp = fopen (volume_path, "r"))
        	{
                ...
                //JAG-203	[Logs][USB] the logs shows wrong when insert or remove the USB device
            	else if (strlen(curUsbDev[i].volumeName) == 1)
                {
                    if(curUsbDev[i].volumeName[0] == '\n' || curUsbDev[i].volumeName[0] == ' ')
            			curUsbDev[i].volumeName[0] = '\0';  /* strip '\n' and ' ' */
                }
                //JAG-203	[Logs][USB] the logs shows wrong when insert or remove the USB device
        	}
...
/* Foxconn modified end, Wins, 04/11/2011 */
{
    ...
    if (rval == 0) {
        system("ps > /tmp/downloader_process_state");
        fp = fopen("/tmp/downloader_process_state", "r");
        if (fp) {
            while (!feof(fp)) {
                fgets(log_buf, sizeof(log_buf), fp);
                if (strstr(log_buf, "amuled")  && strstr(log_buf, path)) {
                    system("/etc/aMule/amule.sh stop");
                    sleep(1);
                    break;
                }
            }
            fclose(fp);
            system("rm /tmp/downloader_process_state");
        }
    }
...
#ifdef BCA_HNDROUTER
    if (part)
        sprintf(devname, "%s%c%d", "sd", 'a' + device, part);
    else
        sprintf(devname, "%s%c", "sd", 'a' + device);
#else
    if (part)
        sprintf(devname, "%s%c%d", driver, 'a' + device, part);
    else
...
#if defined(__CONFIG_DLNA__) && !defined(BCA_HNDROUTER)
static void
start_dlna()
{
