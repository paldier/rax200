 ...
 * $Id: cgi_common.c 764765 2018-05-31 20:12:33Z $
 */
...
{
#ifdef BCA_HNDROUTER
	char default_fw_path0[] = "/data/rtecdc0.bin";
	char default_fw_path1[] = "/data/rtecdc1.bin";
	char default_fw_path2[] = "/data/rtecdc2.bin";
#else
	char default_fw_path0[] = "/media/nand/rtecdc0.bin";
	char default_fw_path1[] = "/media/nand/rtecdc1.bin";
	char default_fw_path2[] = "/media/nand/rtecdc2.bin";
#endif // endif
	...
	if (!strcmp(wl_unit, "0") &&
		(!(fw_path = nvram_safe_get("firmware_path0")) ||
		!strcmp(fw_path, ""))) {
		fw_path = default_fw_path0;
		/* set firmware_path0 nvram after download ok */
		nvram_set("firmware_path0", fw_path);
		nvram_commit();
	}
	else if (!strcmp(wl_unit, "1") &&
		(!(fw_path = nvram_safe_get("firmware_path1")) ||
		!strcmp(fw_path, ""))) {
		fw_path = default_fw_path1;
		/* set firmware_path1 nvram after download ok */
		nvram_set("firmware_path1", fw_path);
		nvram_commit();
	}
	else if (!strcmp(wl_unit, "2") &&
		(!(fw_path = nvram_safe_get("firmware_path2")) ||
		!strcmp(fw_path, ""))) {
		fw_path = default_fw_path2;
		/* set firmware_path2 nvram after download ok */
		nvram_set("firmware_path2", fw_path);
		nvram_commit();
	}
	else {
		cprintf("exist fw_path %s for wl_unit %s \n", fw_path, wl_unit);
	}

	/* cp binary to dest firmware path */
	cp_argv[2] = fw_path;
	_eval(cp_argv, NULL, 0, NULL);
...
ej_wl_dongle_fw_upload(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if (!strcmp(wl_unit, "0")) {
		...
		websWrite(wp,"	(Debug only- Not intended for customer UI)&nbsp;&nbsp;<br>Dongle firmware_path0:&nbsp;&nbsp;");
		websWrite(wp,"    </th>");
		websWrite(wp,"    <td>&nbsp;&nbsp;</td>");
		websWrite(wp,"    <td><input name=\"firmware_path0\" value=\"%s\" size=\"48\" maxlength=\"48\"></td>", nvram_safe_get("firmware_path0"));
		websWrite(wp,"  </tr>");
	}
	else if (!strcmp(wl_unit, "1")) {
		websWrite(wp,"  <tr>");
		...
		websWrite(wp,"	onMouseOver=\"return overlib('Displays the current dongle firmware path (set NULL to use embedded fw).', LEFT);\"");
		websWrite(wp,"	onMouseOut=\"return nd();\">");
		websWrite(wp,"	(Debug only- Not intended for customer UI)&nbsp;&nbsp;<br>Dongle firmware_path1:&nbsp;&nbsp;");
		websWrite(wp,"    </th>");
		websWrite(wp,"    <td>&nbsp;&nbsp;</td>");
		websWrite(wp,"    <td><input name=\"firmware_path1\" value=\"%s\" size=\"48\" maxlength=\"48\"></td>", nvram_safe_get("firmware_path1"));
		websWrite(wp,"  </tr>");
	}
	else if (!strcmp(wl_unit, "2")) {
		websWrite(wp,"  <tr>");
		...
		websWrite(wp,"	onMouseOver=\"return overlib('Displays the current dongle firmware path (set NULL to use embedded fw).', LEFT);\"");
		websWrite(wp,"	onMouseOut=\"return nd();\">");
		websWrite(wp,"	(Debug only- Not intended for customer UI)&nbsp;&nbsp;<br>Dongle firmware_path2:&nbsp;&nbsp;");
		websWrite(wp,"    </th>");
		websWrite(wp,"    <td>&nbsp;&nbsp;</td>");
		websWrite(wp,"    <td><input name=\"firmware_path2\" value=\"%s\" size=\"48\" maxlength=\"48\"></td>", nvram_safe_get("firmware_path2"));
		websWrite(wp,"  </tr>");
	}
	websWrite(wp,"</table>");
	websWrite(wp,"<p>");
...
validate_bridge(webs_t wp, char *value, struct variable *v, char *varname )
{
	...
	memset(buf,0,sizeof(buf));
	strncpy(buf, nvram_get(add_br),sizeof(buf) - 1);
...
validate_wl_akm(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	if (!strcmp(brcm_psk, "enabled"))
		strncat(akms, "brcm_psk ", WLC_IOCTL_SMLEN - strlen(akms) - 1);
...
*/
...
#endif /* __CONFIG_EMF__ */
	{ "wl_pspretend_retry_limit", "PsPretend retry limit", wl_prefix, validate_range, ARGV("0", "32"), FALSE, NVRAM_MI | VIF_IGNORE },
	...
	{ "wl_txbf_bfr_cap", "TX Beamformer", wl_prefix, validate_choice, ARGV("0", "1", "2"), FALSE,NVRAM_MI | VIF_IGNORE |  EZC_FLAGS_READ | EZC_FLAGS_WRITE },
	{ "wl_txbf_bfe_cap", "TX Beamformee", wl_prefix, validate_choice, ARGV("0", "1", "2"), FALSE,NVRAM_MI | VIF_IGNORE |  EZC_FLAGS_READ | EZC_FLAGS_WRITE },
...
REG_EJ_HANDLER(as_cer_display);

/* request new user certificate */
static int
cert_ask(char *name, uint32 period, char *ret_msg, int msg_size)
{
	char cmd[100];
	char *rbuf = NULL;
	int rbuf_len = 128;
	...
	rbuf = malloc(rbuf_len);
	if (as_communicate(cmd, strlen(cmd), rbuf, &rbuf_len)) {
		snprintf(ret_msg, msg_size, "Communicate to AS server failed");
	}
	else {
		if (rbuf_len == 0)
			snprintf(ret_msg, msg_size, "Apply new certificate failed");
		else {
...
do_user_x509_cert_dl_cgi(char *url, FILE *stream)
{
	...
	if (cert_ask(owner, period, rcv_msg, LENGTH)) {
...
do_cert_ul_post(char *url, FILE *stream, int len, char *boundary)
{
	...
	if (len == 0 || len == -1) {
		strncpy_n(posterr_msg, "Invalid AS certificate file<br>", ERR_MSG_SIZE);
		return;
	}
	...
	if (len == 0 || len == -1) {
		strncpy_n(posterr_msg, "Invalid user certificate file<br>", ERR_MSG_SIZE);
		return;
	}

	/* read as and user certificate */
	if ((as_data = read_all_data(AS_CERFILE_PATH, -1, -1)) == NULL) {
		strncpy_n(posterr_msg, "Read AS certificate file fail<br>", ERR_MSG_SIZE);
	...
	if((check_cert_file(user_data)) != 0) {
		strncpy_n(posterr_msg, "Certificate file format error.<br>", ERR_MSG_SIZE);
		goto do_cert_ul_post_error;
	}
	...
	if (bufl == 0) {
		strncpy_n(posterr_msg, "X509 certificate converge fail.<br>", ERR_MSG_SIZE);
		goto do_cert_ul_post_error;
	}
	...
	if (ret == 0) {
		printf("\nAS IP_addr error!!!\n\n");
		strncpy_n(posterr_msg, "System error. installing certificate failed.<br>",
			ERR_MSG_SIZE);
		goto do_cert_ul_post_error;
	}
	...
	if (ret != 0) {
		...
		strncpy_n(posterr_msg, "WAI is disabled.<br>", ERR_MSG_SIZE);
		goto do_cert_ul_post_error;
	}
	...
	if (ret) {
		strncpy_n(posterr_msg, "Save certificate error.<br>", ERR_MSG_SIZE);
		goto do_cert_ul_post_error;
	}
