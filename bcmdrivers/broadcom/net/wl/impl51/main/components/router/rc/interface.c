...
#ifdef VLAN_SUPPORT
extern int getVlanRule(vlan_rule vlan[C_MAX_VLAN_RULE]); /* Foxconn added by Max Ding, 11/22/2016 */
#endif
    ...
    if(strlen(ports))
    {
        ...
        char priority_command[64];
   	    foreach(port_str, ports, next) {
            ...
            sprintf(priority_command,"ethswctl -c regaccess -v 0x%X -d 0x%X -l 2", 0x3410 + port * 2, vlan_id + (priority << 13));
...
start_vlan(void)
{
...
#ifdef BCA_HNDROUTER
	vlan_rule vlan[C_MAX_VLAN_RULE]={0};
	int numVlanRule = 0;
	char vlan_ifname[32];
	char skb_pri[8];
#else
	...
	/* Foxconn add start, Max Ding, 11/22/2016 */
#ifdef BCA_HNDROUTER
	numVlanRule = getVlanRule(vlan);
	
	ifconfig(WAN_IF_NAME_NUM, IFUP, 0, 0); //it is eth0 usually
	
#ifdef INCLUDE_IPV6
	sprintf(buf, "echo 0 > /proc/sys/net/ipv6/conf/%s/router_solicitations", WAN_IF_NAME_NUM);
	system(buf);
#endif
	...
	for (i=0; i<numVlanRule; i++) {
		if (!strcmp(vlan[i].enable_rule, "0"))
			continue;
		eval("vconfig", "add", WAN_IF_NAME_NUM, vlan[i].vlan_id);
		
		snprintf(vlan_ifname, sizeof(vlan_ifname), "vlan%s", vlan[i].vlan_id);
		ifconfig(vlan_ifname, IFUP, 0, 0);
		
#ifdef INCLUDE_IPV6
		sprintf(buf, "echo 0 > /proc/sys/net/ipv6/conf/%s/router_solicitations", vlan_ifname);
		system(buf);
#endif
		
		for (j = 0; j < VLAN_NUMPRIS; j ++) {
			sprintf(skb_pri, "%d", j);
			eval("vconfig", "set_ingress_map",vlan_ifname, skb_pri, skb_pri);
#ifdef CONFIG_ORANGE_ISP
			if(j==6 && nvram_match("orange_isp_enabled", "1"))
			    eval("vconfig", "set_egress_map",vlan_ifname, skb_pri, "6");
			else
			    eval("vconfig", "set_egress_map",vlan_ifname, skb_pri, "0");
#else			    
			eval("vconfig", "set_egress_map",vlan_ifname, skb_pri, vlan[i].vlan_prio);
#endif
		}
	}
#else
