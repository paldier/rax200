 ...
 *
 * $Id: httpd.h 526818 2015-01-15 03:19:01Z $
 */
...
#define websWrite(wp, fmt, args...) \
	({ int TMPVAR = fprintf(wp, fmt , ## args); \
	fflush(wp); TMPVAR; })
...
#define websDefaultHandler(wp, urlPrefix, webDir, arg, url, path, query) \
	({ do_ej(path, wp); fflush(wp); 1; })
#define websWriteData(wp, buf, nChars) \
	({ int TMPVAR = fwrite(buf, 1, nChars, wp); fflush(wp); TMPVAR; })
