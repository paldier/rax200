 ...
 *
 * $Id: dhd_wet.c 766723 2018-08-10 09:15:07Z $
 */
	...
	uint8 *eh, uint8 *iph, uint8 *icmp6h, int length, int send)
{
	...
	if (send) {
		...
		for (icmp6hdlr = &icmpv6_hash[WET_ICMPV6_HASH(type)];
			...
			return (icmp6hdlr->icmpv6_proc)(weth, ifidx, eh, iph, icmp6h, length, send);
		}
	}
	...
	uint8 *eh, uint8 *iph, uint8 *icmp6h, int length, int send)
{
	 ...
	 */
	if (send) {
		/*	Until now there are two options for this,
			but usually target link-layer address is the first option
		*/
		...
		uint8 *iaddr, struct ether_addr *eaddr, wet_sta_t **saddr)
{
	...
	/* bail out if we can't find nor create any */
		...
		DHD_INFO(("wet_sta_update_all: unable to alloc STA ipv%d, %u.%u.%u.%u %s\n",
