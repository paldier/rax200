 ...
 *
 * $Id: dhd_msgbuf.c 767734 2018-09-25 04:29:08Z $
 */
	...
	dhd_pkttype_t pkttype)
{
	...
	*secdma = DHD_PKT_GET_SECDMA(pktptr);
    ...
    link_node = (dll_t *)&(DHD_PKTTAG_FD(pktptr)->node);
    if (!virt_addr_valid((unsigned long)(link_node->next_p)) ||
        !virt_addr_valid((unsigned long)(link_node->prev_p))) {
        printk("%s: invalid addr detected: pktptr %p next_p %p prev_p %p pkttype %d\n",
            __FUNCTION__, pktptr, link_node->next_p, link_node->prev_p, pkttype);
        return NULL;
    }
...
int dhd_prot_ioctl(dhd_pub_t *dhd, int ifidx, wl_ioctl_t *ioc, void *buf, int len)
{
    ...
    if (dhd->ioctl_long_timeout) {
        dhd->ioctl_long_timeout = 0;
    }
...
dhd_msgbuf_wait_ioctl_cmplt(dhd_pub_t *dhd, uint32 len, void *buf)
{
    ...
    if ((timeleft == 0) && (prot->ioctl_received == 0)) {
		dhd->rxcnt_timeout++;
		dhd->rx_ctlerrs++;
