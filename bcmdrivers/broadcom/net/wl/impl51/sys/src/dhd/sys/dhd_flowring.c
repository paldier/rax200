...
dhd_flowid_update(dhd_pub_t *dhdp, uint8 ifindex, uint8 prio, void *pktbuf)
{
	...
	if (dhd_flowid_lookup(dhdp, ifindex, prio, eh->ether_shost, pktdata, &flowid) != BCME_OK) {
#else
                               ...
                               uint8 op, uint8 role)
{
	...
	if (op == WLC_E_IF_ADD || op == WLC_E_IF_CHANGE) {
		...
		if (role == WLC_E_IF_ROLE_WDS) {
			...
			DHD_INFO(("%s: Flow ring for ifindex %d role is %d \n",
...
dhd_idma_flowmgr_init(dhd_pub_t *dhdp, int max_h2d_rings)
{
	...
	/* We can use 15 iDMA set for H2D WR, each set fully packed with 32 indices 64B */
	map = flowmgr->maps;

	/* First id16 map allocator only contains
	 * BCMPCIE_IDMA_FLOWS_PER_SET - FLOWID_RESERVED flows.
	 */
	ASSERT(BCMPCIE_IDMA_FLOWS_PER_SET > FLOWID_RESERVED);
	max_flows = BCMPCIE_IDMA_FLOWS_PER_SET - FLOWID_RESERVED;
	map->max = max_flows;
	...
	max_flows = BCMPCIE_IDMA_FLOWS_PER_SET;
...
dhd_idma_flowmgr_free(void *mgr, uint16 flow_id)
{
	...
	index = flow_id / BCMPCIE_IDMA_FLOWS_PER_SET;
	ASSERT(index < BCMPCIE_IDMA_H2D_MAX_DESC);
...
/* See bcmpcie.h */
uint16
dhd_idma_flowmgr_ringid_2_index(void *mgr, uint16 ringid, uint16 flow_id)
{
	uint16 uindex;
	...
	uindex = flow_id / BCMPCIE_IDMA_FLOWS_PER_SET;
...
static INLINE void dhd_flowid_map_free(dhd_pub_t *dhdp, void *flowid_map, uint16 flowid)
{
