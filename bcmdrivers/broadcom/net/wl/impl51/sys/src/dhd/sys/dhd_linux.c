 ...
 *
 * $Id: dhd_linux.c 768100 2018-10-04 09:35:20Z $
 */
...
#ifdef BCM_DHD_FPM_LEAK_WAR
#include <fpm.h>
#include <pcie_core.h>
#include <dhd_pcie.h>
#endif /* BCM_DHD_FPM_LEAK_WAR */
...
enum macdbgdumptype {
	...
	MACDBG_DUMP_SCPSM	= 4,
	MACDBG_DUMP_RLMEM	= 5,
	MACDBG_DUMP_PHYSVMP	= 6,
	MACDBG_DUMP_RXBM	= 7,
	MACDBG_DUMP_BMC		= 8,
	MACDBG_DUMP_PERUSER	= 9,
	MACDBG_DUMP_MAX
};

#define MACDBGDUMPNAME {				\
	"mac", "mac1", "macx", "sctpl",			\
	"scpsm", "ratelinkmem", "physvmp", "rxbm",	\
	"bmc", "peruser",				\
}
...
dhd_fwder_del_sta(dhd_pub_t *my_dhdp, void *ea)
{
 ...
 #if defined(CONFIG_SMP)
            if ((on_cpu != my_cpu) && (locked_offline_cpu == -1))
                DHD_PERIM_LOCK_ALL(on_cpu);
#endif
			dhd_pub_del_sta(on_dhdp, ea);
#if defined(CONFIG_SMP)
            if ((on_cpu != my_cpu) && (locked_offline_cpu == -1))
                DHD_PERIM_UNLOCK_ALL(on_cpu);
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if (ifp->wmf.wmf_enable && (ETHER_ISMULTI(eh->ether_dhost))) {
            ...
            if (!sta)
                DHD_ERROR(("%s: dhd%d.%d can't find sta "MACF"\n", __func__,
                    dhdp->info->unit, ifidx, ETHERP_TO_MACF(eh->ether_shost)));
...
#if defined(BCM_DHD_RUNNER) && defined(BCM_DHD_FPM_LEAK_WAR)
/* watchdog timer routine calls arp update routine per interface every 1 sec */
static void
dhd_fpm_monitor(dhd_pub_t *dhdp)
{
	struct fpm_pool_stats stats;

	dhdp->fpm_monitor_cnt++;
	if (dhdp->fpm_monitor_cnt != BCM_FPM_UPDATE_TIMEOUT)
	    return;

	fpm_get_pool_stats(1, &stats);

	if (stats.tok_avail < BCM_FPM_LOWMARK) {
		dhdp->fpm_keep_low_cnt++;
		if (dhdp->fpm_keep_low_cnt == BCM_FPM_LOWMARK_THRESH) {
			if (dhdp->busstate == DHD_BUS_DATA) {
				DHD_ERROR(("%s: fpm_keep_low_cnt %d, trap for restart!\n",
					__FUNCTION__, dhdp->fpm_keep_low_cnt));
				dhdp->fpm_keep_low_cnt = dhdp->fpm_monitor_cnt = 0;

				dhdpcie_send_mb_data(dhdp->bus, PCIE_IPC_H2DMB_FW_TRAP);
			}
			else {
				DHD_ERROR(("%s: fpm_keep_low_cnt %d but bus is NOT up\n",
					__FUNCTION__, dhdp->fpm_keep_low_cnt));
			}
		}
	} else
		dhdp->fpm_keep_low_cnt = 0;

	dhdp->fpm_monitor_cnt = 0;
}
#endif /* BCM_DHD_RUNNER && BCM_DHD_FPM_LEAK_WAR */
...
#if defined(BCM_DHD_RUNNER) && defined(BCM_DHD_FPM_LEAK_WAR)
				if (DHD_RNR_OFFL_TXSTS(&dhd->pub))
					dhd_fpm_monitor(&dhd->pub);
#endif
...
dhd_os_ioctl_resp_wait(dhd_pub_t *pub, uint *condition)
{
    ...
    if (pub->ioctl_long_timeout) {
        timeout = msecs_to_jiffies(7000);
    }
...
_dhd_macdbg_dump_name(dhd_info_t *dhd, char* dumpname, char *dumpbuf)
{
	...
	int res, i;
	const char *macdbg_dump_name[MACDBG_DUMP_MAX] = MACDBGDUMPNAME;

	/* validation if this dump is allowed */
	for (i = 0; i < MACDBG_DUMP_MAX; i++) {
		if (strncmp(macdbg_dump_name[i], dumpname, strlen(dumpname)) == 0) {
			break;
		}
	}
	if (!isbitset(dhd->macdbg_dump_level, i)) {
