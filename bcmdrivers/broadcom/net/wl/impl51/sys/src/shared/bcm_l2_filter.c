 ...
 *
 * $Id: bcm_l2_filter.c 743566 2018-01-29 06:06:35Z $
 *
 */
#include <bcmutils.h>
#include <bcmendian.h>
...
bcm_l2_filter_gratuitous_arp(osl_t *osh, void *pktbuf)
{
	...
	if (ethertype == ETHER_TYPE_ARP) {
		L2_FILTER_MSG(("%s ARP RX data : %p: datalen : %d\n", __FUNCTION__,
			data, datalen));
	...
	if (bcmp(data + send_ip_offset, data + target_ip_offset, iplen) == 0) {
		L2_FILTER_MSG((" returning BCME_OK in %s\n", __FUNCTION__));
		return BCME_OK;
	}
	...
	uint8 **data_ptr, int *len_ptr, uint16 *et_ptr, bool *snap_ptr)
{
	...
	if (length < ETHER_HDR_LEN) {
		L2_FILTER_MSG(("BCM: %s: short eth frame (%d)\n",
		           __FUNCTION__, length));
		return BCME_ERROR;
		...
		L2_FILTER_MSG((" %s: non-SNAP 802.3 frame\n", __FUNCTION__));
		return BCME_ERROR;
	}
	...
	if (ethertype == ETHER_TYPE_8021Q) {
		...
		if ((pt + ETHER_TYPE_LEN) > (frame + length)) {
			L2_FILTER_MSG(("BCM: %s: short VLAN frame (%d)\n",
			          __FUNCTION__, length));
			return BCME_ERROR;
		}
		ethertype = ntoh16_ua(pt);
	...
	uint8 **data_ptr, int *len_ptr, uint8 *prot_ptr)
{
	...
	if (ippktlen < iplen) {
		L2_FILTER_MSG(("%s: extra frame length ignored\n",
		          __FUNCTION__));
		iplen = ippktlen;
	} else if (ippktlen > iplen) {
		L2_FILTER_MSG(("%s: truncated IP packet (%d)\n",
		           __FUNCTION__, ippktlen - iplen));
		return BCME_ERROR;
	}

	if (iphdrlen < IPV4_OPTIONS_OFFSET || iphdrlen > iplen) {
		L2_FILTER_ERROR((" %s: IP-header-len (%d) out of range (%d-%d)\n",
	...
	if ((iph_frag & IPV4_FRAG_MORE) || (iph_frag & IPV4_FRAG_OFFSET_MASK) != 0) {
		L2_FILTER_ERROR(("%s: IP fragment not handled\n",
		           __FUNCTION__));
		return BCME_ERROR;
	}
	prot = IPV4_PROT(iph);
	...
	int ifidx, uint8** mac_addr)
{
	...
	if (udpl < UDP_HDR_LEN) {
		L2_FILTER_MSG(("BCM: %s: short UDP frame, ignored\n",
		    __FUNCTION__));
		return BCME_ERROR;
	}
	...
	if (dhcpl < DHCP_CHADDR_OFFSET + ETHER_ADDR_LEN) {
		L2_FILTER_MSG(("BCM: %s: short DHCP frame, ignored\n",
		    __FUNCTION__));
		return BCME_ERROR;
	}
	...
	struct ether_addr *dst_ea, uint16 ea_type, bool snap, void **p)
{
	...
	if ((pkt = PKTGET(osh, pktlen, FALSE)) == NULL) {
		L2_FILTER_ERROR(("%s %d: PKTGET failed\n", __FUNCTION__, __LINE__));
		return NULL;
	}
...
bcm_l2_filter_block_tdls(osl_t *osh, void *pktbuf)
{
	...
	if (datalen < TDLS_PAYLOAD_TYPE_LEN + 2) {
		L2_FILTER_ERROR(("%s: wrong length for 89-0d eth frame %d\n",
	...
	if (*data != TDLS_PAYLOAD_TYPE) {
		L2_FILTER_ERROR(("%s: wrong payload type for 89-0d eth frame %d\n",
				__FUNCTION__, *data));
		return BCME_ERROR;
	}
	...
	if (*data != TDLS_ACTION_CATEGORY_CODE) {
		L2_FILTER_ERROR(("%s: wrong TDLS Category %d\n", __FUNCTION__, *data));
		return BCME_ERROR;
	}
	data++;
