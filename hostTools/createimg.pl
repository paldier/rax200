...
my $arg_mapper = {
    ...
    'misc5' => sub { my $dstruct=shift; my $val=shift; $dstruct->{'part_info'}[4]{'size'}=$val},
    'misc6' => sub { my $dstruct=shift; my $val=shift; $dstruct->{'part_info'}[5]{'size'}=$val},
    'misc7' => sub { my $dstruct=shift; my $val=shift; $dstruct->{'part_info'}[6]{'size'}=$val},
    'misc8' => sub { my $dstruct=shift; my $val=shift; $dstruct->{'part_info'}[7]{'size'}=$val},
    'misc9' => sub { my $dstruct=shift; my $val=shift; $dstruct->{'part_info'}[8]{'size'}=$val},
    'misc10' => sub { my $dstruct=shift; my $val=shift; $dstruct->{'part_info'}[9]{'size'}=$val},
...
else
{
	...
	if($csum == $wfi_tag_struct->{'wfiCrc'})
	{
		if($wfi_tag_struct->{'wfiChipId'} == 0x63148 || 
            ...
            $wfi_tag_struct->{'wfiChipId'} == 0x6858 ||
			$wfi_tag_struct->{'wfiChipId'} == 0x4908 || 
			$wfi_tag_struct->{'wfiChipId'} == 0x63158 || 
            $wfi_tag_struct->{'wfiChipId'} == 0x6836 )
		{
