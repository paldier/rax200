...
static int do_bridge_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	u32 tmp;
	...
	for (i = 0; i < 3; i++) {
		if (get_user(tmp, i + ((u32 *)arg)) || put_user(tmp, i + argbuf))
			return -EFAULT;
	}
	return sock_ioctl(file, cmd, (unsigned long)argbuf);
}
/* Foxconn */
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	case SIOCSIFBR:
	case SIOCGIFBR:
		return do_bridge_ioctl(file, cmd, arg);
