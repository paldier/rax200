...
#if (defined BCA_HNDROUTER) || (defined CONFIG_QCA_SDK)
...
#endif
    iptables("-A EXT_FORWARD_IN_CHAIN -j FW_UPNP_PORTMAPPING");
    iptables("-t nat -A PREROUTING -j FW_NAT_UPNP_PRIOTIY_2_3");
        ...
        if (destips != NULL) for (destip = strtok_r(strtok25_FREE, local25IFS, &r_destips1); destip != NULL; destip = strtok_r(NULL, local25IFS, &r_destips1))
        {
          ...
          if (interfaces != NULL) for (eif = strtok_r(strtok26_FREE, local26IFS, &r_interfaces1); eif != NULL; eif = strtok_r(NULL, local26IFS, &r_interfaces1))
          {
...
#if (defined BCA_HNDROUTER) || (defined CONFIG_QCA_SDK) /* Foxconn added by Max Ding, 02/24/2017 */
            ...
            /* foxconn add end, for UDP port scan drop from erouter0, Jonathan 2013/5/2 */
              ...
              /* Foxconn add by Silver, for DMZ log, 2017/3/27 */
              ip4tables("-A EXT_FORWARD_IN_CHAIN -i %s ! -o %s -s %s -d %s -p tcp --dport %s -j LOG --log-prefix PORT_FORWARD --log-level 5", eif, eif, shost, dhost, dport);
...
//# DMZ Non-TCP/UDP port forwards (protocol based)
  ...
  system("/usr/sbin/agApi_fwSetPortForwardByNvram.sh");
  system("/usr/sbin/agApi_fwSetRemoteService.sh");
...
//}
    ...
    iptables("-N FW_UPNP_PORTMAPPING");
    iptables("-t nat -N FW_NAT_UPNP_PRIOTIY_2_3");
...
//}
  ...
  iptables("-F SYN_FLOOD_CHK");
    ...
    iptables("-F FW_UPNP_PORTMAPPING");
    iptables("-t nat -F FW_NAT_UPNP_PRIOTIY_2_3");
...
{
    printf("Reading custom rules from %s !!!\n", "/var/custom_service_rules");
    source("/var/custom-service-rules");
}
#endif
