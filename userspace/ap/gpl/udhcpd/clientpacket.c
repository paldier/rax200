...
static void gen_option90_string(char* option_str)
{
    ...
    extern char option90_autheticationId[64];
    int len;
    ...
    {
        memset(&dhcp90_content, 0, sizeof(struct dchp_option_90_auth_id));
	    ...
	    strncpy(dhcp90_content.authentication_info, option90_autheticationId, strlen(option90_autheticationId));

        dhcp90_content.length = sizeof(dhcp90_content.protocol) + sizeof(dhcp90_content.algorithm) \
            + sizeof(dhcp90_content.RDM) + sizeof(dhcp90_content.replay_detection) + strlen(dhcp90_content.authentication_info);
...
static void init_packet(struct dhcpMessage *packet, char type)
{
    ...
    if(strlen(service_type)>0 && strstr(service_type, "DHCP"))
        gen_option77_string(userClass1);
    
    /* @option90 authentication id */
    unsigned char autheticationId1[66];
    memset(autheticationId1, 0, sizeof(autheticationId1));
    if(strlen(service_type)>0)
