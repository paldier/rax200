...
#if defined(_XDSL_PRODUCT) || defined(CONFIG_ORANGE_ISP)
char option90_autheticationId[64] = ""; /*@option90 */
...
#if (defined IOCTL_NO_POINTER)
#define IOCTL_PARAM     T_agIoctlParamPkg
#else /* !IOCTL_NO_POINTER */    
#define IOCTL_PARAM     char*
#endif /* !IOCTL_NO_POINTER */ 

#define MAJOR_NUM 100
#define AG_MAX_ARG_CNT	    64
#define AG_MAX_ARG_LEN    100 
#define DEVICE_FILE_NAME "acos_nat_cli"
#define DEV_FILE_NAME_W_PATH    "/dev/"DEVICE_FILE_NAME
#define IOCTL_AG_FW_LISTEN_PORT_SET         _IOR(MAJOR_NUM, 61, IOCTL_PARAM)
...
#ifdef IOCTL_NO_POINTER    
    char       param[AG_MAX_ARG_CNT][AG_MAX_ARG_LEN];
#else /* !IOCTL_NO_POINTER */    
...
int agApi_setDeviceListenPort(int port_idx, int port_num, int enable)
{
...
printf("port_idx: %d, port_num: %d, enable: %d\n", port_idx, port_num, enable);
    ...
    paramPkg.paramCnt = 3;
#ifdef IOCTL_NO_POINTER 
    memcpy(paramPkg.param[0], (char *)&port_idx, sizeof(int));
    memcpy(paramPkg.param[1], (char *)&port_num, sizeof(int));
    memcpy(paramPkg.param[2], (char *)&enable, sizeof(int));
#else /* !IOCTL_NO_POINTER */
    ...
    /* workaround-start for R8000P Mantis #17088-2, #17092-3 */
    /* before iptable is ready, temp return 0 when ret_val < 0*/
    if(ret_val < 0)
    {
        printf("\n%s (%d) ret_val:%d\n",__FUNCTION__,__LINE__, ret_val); 
        return 0;
    }
    /* workaround-end for R8000P Mantis #17088-2, #17092-3 */
    
    close(file_desc);
    
    /* Foxconn add start, Vanessa Kuo, 12/27/2006 */
    if (paramPkg.paramCnt <= -1)
        ret_val = paramPkg.paramCnt;
    /* Foxconn add end, Vanessa Kuo, 12/27/2006 */

    //arnonat_addInputListenPort(ret_val, port_idx, port_num, enable);
...
#define DHCPC_TIMEOUT_RESTART_INTERVAL		10
	...
	while (1) {
...
#if defined(_XDSL_PRODUCT) || defined(CONFIG_ORANGE_ISP)
		c = getopt_long(argc, argv, "c:fbH:h:i:np:qr:s:o:t:vl", arg_options, &option_index);
#else
		...
		case 'o':
			...
			if(strncmp(optarg, "90", 2)==0) {
			    strncpy(option90_autheticationId, optarg+3, sizeof(option90_autheticationId));
			}
			break;
	    case 't':
            strncpy(service_type, optarg, sizeof(service_type));
	        break;
#endif
		case 'v':
