...
dns_dyn_list_t *dns_dyn_find(dns_request_t *m)
{
  ...
  while((ret = dns_dyn_list_find(dns_dyn_cache, cname, paddr))) {
    if (ret->expiry > time(NULL))
      goto found;
    rm = dns_dyn_list_remove(&dns_dyn_cache, ret);
    dns_dyn_cache_count -= rm;
  }
...
/*****************************************************************************/
int dns_dyn_cache_add_multi_answer(dns_request_t *m)
{
  char *addr;
  int i;
  int ret = 0;
  ...
  debug("question[0].type = %d, question[0].name = %s", m->message.question[0].type, m->cname);

  debug("m->message.header.ancount = %d", m->message.header.ancount);

  /* set according to the answer */
  for( i = 0; i < m->message.header.ancount && i < NUM_RRS; i++){
    debug("m->message.answer[%d].type = %d", i, m->message.answer[i].type);
    ...
    if( m->message.question[0].type == m->message.answer[i].type ){
      ...
      debug("m->ip = %s, m->cname = %s", m->ip, m->cname);

      ret = dns_dyn_cache_add(m);
    } /* if( question == answer ) */
  } /* for */
  return ret;
}
