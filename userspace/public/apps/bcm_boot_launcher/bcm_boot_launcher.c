...
void run_list()
{
   ...
   /* Foxconn add start by Finerain Zhou, 04/15/2014 */
   /*Run foxconn script at last*/
   if (start)
   {
       snprintf(cmdline, sizeof(cmdline)-1, "/etc/init.d/foxconn_config.sh" );
       if (0 != (rc = system(cmdline)))
       {
          printf("warning: %s returned %d\n", cmdline, rc);
       }
   }
   /* Foxconn add end by Finerain Zhou, 04/15/2014 */
}
