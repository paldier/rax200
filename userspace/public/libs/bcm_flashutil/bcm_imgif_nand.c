...
typedef struct
{
    ...
    UBOOL8 writingUbifs;
#ifdef UPGRADE_WITHOUT_CFERAM
	UINT32 vmlinuxSize;
	UINT32 flashBlkVmlinux;
	UINT8 *vmlinuxP;
#endif
    UBOOL8 lastB;
    UBOOL8 tagFound;
...
#ifdef UPGRADE_WITHOUT_CFERAM 
static imgif_ret_e procFlashVmlinux(imgif_ctx_t *ctxP);
static void eraseVmlinuxBlock(imgif_ctx_t *ctxP);
static int getFlashCferamBlk(imgif_ctx_t *ctxP);
static imgif_ret_e procSearchVmlinux(imgif_ctx_t *ctxP);
#endif
...
int imgif_nand_get_flash_info(imgif_flash_info_t *flashInfoP)
{
    ...
    imgif_debug("Enter, h=%p, info=%p.", h, flashInfoP);
...
#ifdef UPGRADE_WITHOUT_CFERAM
	/* Flash the fisrt block of vmlinux  */
	imgifRet = procFlashVmlinux(ctxP);
	if (imgifRet == IMGIF_PROC_ERR)
	{
		imgif_error("procFlashVmlinux() failed, ret=%d.", imgifRet);
		goto imgif_close_exit;
	}
#endif
    /* Flash cferam. */
...
static void eraseVmlinuxBlock(imgif_ctx_t *ctxP)
{
    ...
    for (blkAddr = ctxP->flashBlkCferam; blkAddr < ctxP->flashBlkVmlinux;
      blkAddr += ctxP->eraseSize)
    {
        imgif_debug("nandEraseBlk() blkAddr=0x%x, "
          "erasesize=%u, flashBlkCferam=0x%x, blkVmlinux=0x%x\n",
          blkAddr, ctxP->eraseSize,ctxP->flashBlkCferam, ctxP->flashBlkVmlinux);
...
static imgif_ret_e procData(imgif_ctx_t *ctxP)
{
    ...
    UINT32 crc = crc_getCrc32(ctxP->startP, procBytes, ctxP->crcBody);
#ifdef UPGRADE_WITHOUT_CFERAM
	int cferamBlk;
	imgif_ret_e vmlinuxFound = IMGIF_PROC_NOT_FOUND;
#endif
...
#ifdef UPGRADE_WITHOUT_CFERAM
			/* search the block which contains vmlinux in flash*/
			cferamBlk =  getFlashCferamBlk(ctxP);

			if (cferamBlk <= 0) {
				/* if no vmlinux is found or vmlinux is in the first block of upgrade mtd, stop upgrade FW */
				ctxP->flashBlkCferam = 0;
				imgif_error("vmlinux can't be found in flash, stop FW upgrade");
				return IMGIF_PROC_ERR;
			} else {
				/* save block offset  */
				ctxP->flashBlkCferam = cferamBlk;
				/* reserve 9 blocks to store the first vmlinux block */
				ctxP->flashBlkVmlinux = ctxP->flashBlkCferam + getFlashOffsetForCferam(ctxP->eraseSize);
				/* flashBlkAddr is the block to start to flash except first vmlinux block  */
				ctxP->flashBlkAddr = ctxP->flashBlkVmlinux;
				printf("flashBlkCferam 0x%x flashBlkVmlinux 0x%x flashBlkAddr 0x%x",
					ctxP->flashBlkCferam, ctxP->flashBlkVmlinux, ctxP->flashBlkAddr);
			}
#else
    ...
    if (curState == IMGIF_UPGRADE_SEARCH)
    {
        ...
        else
        {
            ...
            {
#ifdef UPGRADE_WITHOUT_CFERAM
				/* search vmlinux, if vmlinux was found, copy the first vmlinux block to vmlinuxP
				*  start to flash the image except for the first vmlinux block
				*/
				vmlinuxFound = procSearchVmlinux(ctxP);
				if (vmlinuxFound == IMGIF_PROC_FOUND)
				{
					moveOn = TRUE;
					ctxP->writingUbifs = FALSE;
					if (ctxP->vmlinuxP &&
						(ctxP->imageType == JFFS2_IMAGE))
					{
						printf("vmlinux is found in JFFS2 image "
							"===> start upgrade from 0x%x.... \n", ctxP->flashBlkAddr);
						eraseVmlinuxBlock(ctxP);
						/* Flash the image except for first vmlinux block */
						newState = IMGIF_UPGRADE_FS;
					
					}
				} else if (ret == IMGIF_PROC_NOT_FOUND) {
					/* Skip the block, if the block is before vmlinux, don't flash it */
					moveOn = TRUE;
				} else
					return IMGIF_PROC_ERR;
#else
...
static int getFlashCferamBlk(imgif_ctx_t *ctxP)
{
	return getFlashVmlinuxOffset(ctxP->mtdP, ctxP->mtdFd, ctxP->eraseSize);
}

static imgif_ret_e procSearchVmlinux(imgif_ctx_t *ctxP)
{
	int vmlinuxOffs;
    imgif_ret_e ret = IMGIF_PROC_NOT_FOUND;

    /*
     * Check and process the block which contains vmlinux.lz
     */
    vmlinuxOffs = imgSearchVmlinux(ctxP->startP, ctxP->eraseSize, ctxP->eraseSize);
    if (vmlinuxOffs < 0) {
		imgif_debug("Not a block containing vmlinux");
    } else {
        /* Found. */
        ctxP->vmlinuxSize = ctxP->eraseSize;
        ctxP->vmlinuxP = malloc(ctxP->vmlinuxSize);
        if (ctxP->vmlinuxP != NULL)
        {
        	/* copy first block of vmlinux, will flash it in imgif_close -> procFlashVmlinux() */
            memcpy(ctxP->vmlinuxP, ctxP->startP, ctxP->vmlinuxSize);
            ret = IMGIF_PROC_FOUND;
        }
        else
        {
            imgif_error("malloc(%d) failed.", ctxP->eraseSize);
...
static imgif_ret_e procFlashVmlinux(imgif_ctx_t *ctxP)
{
    ...
    if (ctxP->vmlinuxP == NULL)
    {
        imgif_error("Invalid vmlinux pointer.");
        return IMGIF_PROC_ERR;
    }

    ret = flashVmlinux(ctxP->mtdP, ctxP->mtdFd, (int)ctxP->flashBlkCferam, (int)ctxP->flashBlkVmlinux,
      ctxP->vmlinuxP);
    if (ret < 0)
    {
        imgif_error("flashVmlinux(%p, 0x%x, %u, %u, %p) failed.",
          ctxP->mtdP, ctxP->mtdFd, ctxP->flashBlkCferam, ctxP->flashBlkVmlinux, ctxP->vmlinuxP);
        return IMGIF_PROC_ERR;
    }

    ctxP->flashedBytes += ctxP->vmlinuxSize;
    ctxP->flashedBlks++;
