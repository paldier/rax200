    ...
    while(fgets(line, sizeof(line), fp))
    {
        ...
        if(!strcmp(name, compare))
        {
            ...
            { // proper writing whereby UBI takes care of erasing can only be done through mtdblock, does not work via mtd
                sprintf(name, "%s%s", "/dev/mtdblock", &mtdn[3]); // get rid of leading "mtd"
        ...
        { // use UBI interface
            ...
            for (i = 0; i < 2; i++)
            {
                ...
                if (parse_ubi(0, buf, 0, mtd->size / len, len, -1, fname, imageSequence, 0, 0, read_block, 0, 0, mtd, mtdblock_fd) == 3)
                {
                    ...
                    printf("ERROR!!! Could not find image %d metadata volume %d\n", imageNumber, i);
...
unsigned char *nandUpdateSeqNum(unsigned char *imagePtr, int imageSize, int blkLen, int seq, int *found)
{
    ...
    if( seq != -1 )
    {
        ...
        for(p = imagePtr; p < imagePtr+imageSize && done == 0; p += blkLen)
        {
            ...
            if( (nand_image_type(p) == UBI_IMAGE) && (getCrc32((void *)ec, UBI_EC_HDR_SIZE-4, CRC_INITIAL_VALUE) == be32_to_cpu(ec->hdr_crc)) )
...
static int findVmlinux(unsigned char *p, int blkLen)
{
	struct jffs2_raw_dirent* de, *dend;
	char fname[] = NAND_FLASH_BOOT_IMAGE_LZ;
	unsigned long version = 0;
	int fname_len = strlen(fname);
	int found = 0;

	de = (struct jffs2_raw_dirent*)p;
	dend = (struct jffs2_raw_dirent*)(p + blkLen);

	/* Assumptions: buf and buf_size must be both aligned to 4 */ 
	while(de < dend) {
		if ((je16_to_cpu(de->magic) == JFFS2_MAGIC_BITMASK) && (je16_to_cpu(de->nodetype) == JFFS2_NODETYPE_DIRENT)) {
		 
			if( je32_to_cpu(de->version) > version  && je32_to_cpu(de->ino) != 0 && (fname_len == de->nsize) &&
				!memcmp(fname, de->name, fname_len)) {
				found = 1;
				break;
...
int imgSearchVmlinux(unsigned char *imagePtr, int imageSize, int eraseSize)
{

	unsigned char *p;
	int ret = -1;
	int blkLen = eraseSize;

	flashutil_debug("imgSearchVmlinux");

	for(p = imagePtr; p < imagePtr+imageSize; p += blkLen)
	{
		if (findVmlinux(p, blkLen) == 1) {
			ret = 1;
			break;
		}
	}
...
int getFlashVmlinuxOffset(mtd_info_t *mtdP, int mtdFd, int eraseSize)
{
	...
	flashutil_debug("getFlashVmlinuxOffset");

    if (NULL == (pBuf = malloc(eraseSize)))
    {
        fprintf(stderr, "Memory allocation failed");
        return ERROR;
    }
	...
	/* Read from boot partition (image), in single image, boot partition is equal to upgrade parition */
	for(block = 0; block < mtdP->size; block += blkLen)
	{
		nandReadBlk(mtdP, block, blkLen, p, mtdFd);
		if (findVmlinux(p, blkLen) == 1) {
			printf("%s is found at block 0x%x \n", NAND_FLASH_BOOT_IMAGE_LZ, block);
			found = 1;
			break;
		}
	}
...
int flashVmlinux(mtd_info_t *mtd, int mtd_fd, int rsrvd_for_cferam, int rsrvd_for_vmlinux, unsigned char *vmlinux_ptr)
{
    int blk_addr;
    int ret = -1;

    flashutil_debug("enter, flashVmlinux mtd=%p, fd=%d, rsrvd_for_cferam=0x%x rsrvd_for_vmlinux=0x%x",
      mtd, mtd_fd, rsrvd_for_cferam, rsrvd_for_vmlinux);

    for( blk_addr = rsrvd_for_cferam; blk_addr < rsrvd_for_vmlinux; blk_addr += mtd->erasesize )
    {
        if( nandIsBadBlk(blk_addr, mtd_fd) == 0 ) 
        {
            /* Write a block of the image to flash. */
            if (nandWriteBlk(mtd, blk_addr, vmlinux_ptr ? mtd->erasesize : 0, vmlinux_ptr, mtd_fd, 1) == 0)
            { /* rsrvd_for_vmlinux number of blocks reserved for vmlinux block in order to 
            		     * guarantee at least one block good, vmlinux block with vmlinux entry written last
            		     * write only one block and leave the rest blank
            		     */
				printf(".");
				fflush(stdout);
...
static int chksum_sbi(unsigned char *pSbi, int *cferom_crc)
{
    ...
    crc = crc_getCrc32(pCrcStart, crcLen, crc);
    ...
    if (hdchk->unauthHdr.magic_1 == BTRM_SBI_UNAUTH_MGC_NUM_1)
    {
        if (hdchk->unauthHdr.magic_2 == BTRM_SBI_UNAUTH_MGC_NUM_2)
        {
            ...
            if ((hdchk->unauthHdr.hdrLen >= sizeof(SbiUnauthHdrBeginning)) && (hdchk->unauthHdr.hdrLen < BTRM_SBI_UNAUTH_HDR_MAX_SIZE))
            {
                ...
                crc = crc_getCrc32((uint8_t*)pHdr, hdchk->unauthHdr.hdrLen-sizeof(uint32_t), crc);
...
int is_nvram_offset(unsigned char *buffer, int offset, int check_nvram_data_sign, int *image_size, PNVRAM_DATA current_nvram)
{
    ...
    if(strncmp((char*)buffer+offset, sign, strlen(sign)) == 0)
    {
        ...
        if(crc == (temp_crc=crc_getCrc32((unsigned char *) (buffer+offset+strlen(sign)), sizeof(NVRAM_DATA), CRC_INITIAL_VALUE)))
        {
            rc=IMG_NVRAM;
