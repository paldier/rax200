...
int getScratchPadFile(char** ppBuf, UINT32 *bufLen)
{
    char* fname = SCRATCH_PAD_FILE_NAME;
    struct stat statbuf;
    int rc, fd;
    int newfile = 0;
    mode_t mode = O_RDWR;
    ...
    if (fd == -1)
    {
         cmsLog_error("failed to open %s error %d", fname, errno);
         return -1;
    }
    ...
    if (rc != 0)
    {
        cmsLog_error("failed to lock file %s fd %d rc %d error %d", fname, fd, rc, errno);
        close(fd);
        return -2;
    }
    ...
    *ppBuf = malloc( *bufLen);
    if (*ppBuf == NULL)
    {    
        cmsLog_error("Failed to allocate %d bytes buf", *bufLen);
        flock(fd, LOCK_UN);
        close(fd);
        return -3;
    }
    ...
    if (!newfile)
    {
        ...
        if (rc!= *bufLen)
        {
            cmsLog_error("read file expect %d bytes but reurn %d bytes", *bufLen, rc);
            flock(fd, LOCK_UN);
            close(fd);
            free(*ppBuf);
            return -4;
    ...
    else
    {
        ...
        if (rc != *bufLen)
        {
            cmsLog_error("bad write on new file, got %d expected %d", rc, *bufLen);
            flock(fd, LOCK_UN);
            close(fd);
            free(*ppBuf);
            return -5;
...
int putScratchPadFile(int fd, char* pBuf, UINT32 bufLen, int update)
{
    ...
    if (update) 
    {
        ...
        if (rc != bufLen)
        {
            cmsLog_error("bad write, got %d expected %d", rc, bufLen);
            sts = -1;
        }
    }
    ...
    if (rc != 0)
    {
        cmsLog_error("failed to unlock fd %d rc %d error %d", fd, rc, errno);
        sts = -2;
    }
...
SINT32 devCtl_flashReadFile(const char *fname, char *buf,  UINT32 bufLen)
{
   ...
   if (rc != 0)
   {
       cmsLog_error("file %s not exit stat rc %d", fname, rc);
   ...
   if (fd < 0)
   {
      cmsLog_error("could not open file %s, errno=%d", fname, errno);
      return CMSRET_INTERNAL_ERROR;
   }
   ...
   if (rc != bufLen)
   {
      cmsLog_error("read error, got %d, expected %d", rc, bufLen);
...
SINT32 devCtl_flashWriteFile(const char *fname, char *buf,  UINT32 bufLen)
{
   ...
   {
      cmsLog_error("could not open %s for write", fname);
      return CMSRET_INTERNAL_ERROR;
   }
   ...
   if (rc < (SINT32) bufLen)
   {
      cmsLog_error("write to %s failed, rc=%d", fname, rc);
      ret = CMSRET_INTERNAL_ERROR;
   }
 ...
 */
SINT32 devCtl_scratchPadList(char *tokBuf, int bufLen)
{
    SINT32 sts;
    UINT32 spDataLen = 0;
    char *pBuf = NULL;
    ...
    fd = getScratchPadFile(&pBuf, &spDataLen);
    if( fd < 0 ) 
        return 0;
    ...
    if(memcmp(((PSP_HEADER)pBuf)->SPMagicNum, MAGIC_NUMBER, MAGIC_NUM_LEN) != 0) 
    {
        ...
        {
            cmsLog_error("putScratchPadFile() failed");
        }
        return 0;
    }
 ...
 *         absolute value of the negative number is the number of bytes needed.
 */
SINT32 devCtl_scratchPadGet(char *tokenId, char *tokBuf, int bufLen)
{
    UINT32 spDataLen = 0;
    PSP_TOKEN pToken = NULL;
    char *pBuf = NULL;
 ...
 * @return  0 - ok, -1 - fail.
 */
SINT32 devCtl_scratchPadSet(char *tokenId, char *tokBuf, int bufLen)
{
    UINT32 spDataLen = 0;
    PSP_TOKEN pToken = NULL;
    char *pBuf = NULL;
    ...
    {
        cmsLog_error("Invalid scratch pad key name. Must start with an ascii letter or number.\n");
        return sts;
    }
    ...
    {
        cmsLog_error("Token name length %d large than maximum length %d\n", tokenNameLen, TOKEN_NAME_LEN-1);
        return sts;
    }
    ...
    {
        cmsLog_error("Scratch pad overflow by %zu bytes.  Information not saved.\n",
 ...
 * @return  0 - ok, -1 - fail.
 */
SINT32 devCtl_scratchPadClearAll(void)
{ 
    UINT32 spDataLen = 0;
    SINT32 sts = -1;
    char *pBuf = NULL;
...
CmsRet devCtl_flashConfigAccess(UINT32 boardIoctl, BOARD_IOCTL_ACTION action, char *string,
                         SINT32 strLen, SINT32 offset, void *data)
{
    ...
    if( action == SCRATCH_PAD )
    {
        ...
        case BOARD_IOCTL_FLASH_LIST:
            ret = devCtl_scratchPadList(data, offset);
            break;

        case BOARD_IOCTL_FLASH_READ:
            ret = devCtl_scratchPadGet(string, data, offset);
            break;

        case BOARD_IOCTL_FLASH_WRITE:
            if( offset == -1 )
                ret = devCtl_scratchPadClearAll();
            else
                ret = devCtl_scratchPadSet(string, data, offset);
            break;
        default:
            cmsLog_error("unhandled BOARD_IOCTL code %d for scratch pad", boardIoctl);
    ...
    else
    {
        ...
        default:
            cmsLog_error("unhandled action %d in BOARD_IOCTL code %d", action, boardIoctl);
        ...
        {
            cmsLog_error("unhandled BOARD_IOCTL code %d", boardIoctl);
            ret = CMSRET_INVALID_ARGUMENTS;
        }
    }
...
{
    return( devCtl_boardIoctl(BOARD_IOCTL_GET_CHIP_ID, 0, NULL, 0, 0, chipId) );
}
...
#endif /* DESKTOP_LINUX */
        return( devCtl_boardIoctl(BOARD_IOCTL_BOOT_IMAGE_OPERATION, 0, NULL, 0, state, NULL) );
}
...
#endif /* DESKTOP_LINUX */
        return( (int) devCtl_boardIoctl(BOARD_IOCTL_BOOT_IMAGE_OPERATION, 0, NULL, 0, BOOT_GET_BOOT_IMAGE_STATE, NULL) );
}

int devCtl_getImageVersion(int partition, char *verStr, int verStrSize)
{
    return( (int) devCtl_boardIoctl(BOARD_IOCTL_BOOT_IMAGE_OPERATION, partition, verStr, verStrSize, BOOT_GET_IMAGE_VERSION, NULL) );
}

extern int getBootedValue(void);
...
#endif /* DESKTOP_LINUX */
        return( (int) devCtl_boardIoctl(BOARD_IOCTL_BOOT_IMAGE_OPERATION, 0, NULL, 1, BOOT_GET_BOOTED_IMAGE_ID, NULL) );
}
...
{ // this call is ok as the "0" value does not call getSequenceNumber which won't work with pureUBI images
    return( (int) devCtl_boardIoctl(BOARD_IOCTL_BOOT_IMAGE_OPERATION, 0, NULL, 0, BOOT_GET_BOOTED_IMAGE_ID, NULL) );
}
...
int fake_board_ioctl(UINT32 command, BOARD_IOCTL_PARMS *ctrlParms)
{
   ...
   case BOARD_IOCTL_FLASH_WRITE:
      ...
      default:
         ret = -EINVAL;
         cmsLog_error("unhandled action %d in BOARD_IOCTL_FLASH_WRITE", ctrlParms->action);
         break;
      }
   ...
   case BOARD_IOCTL_FLASH_READ:
      ...
      default:
         ret = -EINVAL;
         ctrlParms->result = ret;
         cmsLog_error("unhandled action %d in BOARD_IOCTL_FLASH_READ", ctrlParms->action);
         break;
      }
   ...
   case BOARD_IOCTL_GET_ID:
      /* this is get BOARD_ID */
      {
         ...
         {
            cmsLog_error("buf too short, need %zu, got %d", strlen(boardIdStr)+1, ctrlParms->strLen);
            ret = -1;
         }
         else
         {
