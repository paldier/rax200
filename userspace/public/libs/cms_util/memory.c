...
#define IS_IN_SHARED_MEM(p) ((((uintptr_t) (p)) >= mStats.shmAllocStart) && \
                             (((uintptr_t) (p)) < mStats.shmAllocEnd))

#endif /* MDM_SHARED_MEM */

void cmsMem_initSharedMem(void *addr, UINT32 len)
{
   mStats.shmAllocStart = (uintptr_t) addr;
   mStats.shmAllocEnd = mStats.shmAllocStart + len;
   mStats.shmTotalBytes = len;

   cmsLog_notice("shm pool: %p-%p", (void *) mStats.shmAllocStart, (void *) mStats.shmAllocEnd);
...
void *cmsMem_realloc(void *origBuf, UINT32 size)
{
   ...
   intBuf = (UINT32 *) (((uintptr_t) origBuf) - CMS_MEM_HEADER_LENGTH);
...
void cmsMem_free(void *buf)
{
   ...
   if (buf != NULL)
   {
      UINT32 *intBuf = (UINT32 *) (((uintptr_t) buf) - CMS_MEM_HEADER_LENGTH);
...
void cmsMem_dumpMemStats()
{
   ...
   printf("Total MDM Shared Memory Region : %dKB\n", (int)((memStats.shmAllocEnd - MDM_SHM_ATTACH_ADDR)/KB_IN_B));
