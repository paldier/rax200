...
#define FLASH_DIE_SELECT    0xC2    /* die select command for Winbond */
...
#define ID_W25M02GV         0xAB

/* MXIC Macronix manufacturer ID */
#define MACRONIXPART        0xC2
#define ID_MX35LF1G         0x12
#define ID_MX35LF2G         0x22
...
#define SPI_NAND_DEVICES                                                    \
    {{SPI_MAKE_ID(GIGADEVPART,  ID_GD5F1GQ4UA),  "GigaDevice GD5F1GQ4UA"},  \
     ...
     {SPI_MAKE_ID(WINBONDPART,  ID_W25M02GV),    "Winbond W25M02GV"},       \
     {SPI_MAKE_ID(MACRONIXPART, ID_MX35LF1G),    "Macronix MX35LF1G"},      \
     {SPI_MAKE_ID(MACRONIXPART, ID_MX35LF2G),    "Macronix MX35LF2G"},      \
...
#if defined(CFG_RAMAPP) || !defined(_CFE_)
...
#endif /* defined(CFG_RAMAPP) || !defined(_CFE_) */
...
//        pchip->chip_ecc_corr = 6; // bad bit threshold to mark page bad (6/8), don't need to set when using enhanced
    ...
    case SPI_MAKE_ID(WINBONDPART, ID_W25M02GV): // 2Gb, 256MB
        pchip->chip_num_blocks = 0x800;  // 2048 blocks total
#if defined(CFG_RAMAPP)
    ...
    case SPI_MAKE_ID(MACRONIXPART, ID_MX35LF1G): // 1Gb, 128MB
#if defined(CFG_RAMAPP)
...
//        pchip->chip_ecc_corr = 3; // bad bit threshold to mark page bad (3/4), don't need to set when using enhanced
    ...
    case SPI_MAKE_ID(MACRONIXPART, ID_MX35LF2G): // 2Gb, 256MB
        pchip->chip_num_blocks = 0x800;  // 2048 blocks total
#if defined(CFG_RAMAPP)
        pchip->ecclayout = &spinand_oob_mxic;
        pchip->chip_ecc = 4; // correctable amount of bits
...
//        pchip->chip_ecc_corr = 6; // bad bit threshold to mark page bad (6/8), don't need to set when using enhanced
    ...
    case SPI_MAKE_ID(ETRONPART, ID_EM73C044): // 1Gb, 128MB
#if defined(CFG_RAMAPP)
        pchip->chip_spare_size = 0x80; // 128, encompasses whole OOB and ECC
    ...
    default: // 1Gb, 128MB
#if defined(CFG_RAMAPP)
        printf("unrecognised spi nand device id 0x%lx, use default cfg setting\n", pchip->chip_device_id);
#endif
        break;
    }
...
#endif
{
    ...
    {
        if (SPI_MANU_ID(pchip->chip_device_id) == WINBONDPART)
        {
            while(!spi_nand_ready());
...
static int spi_nand_dev_specific_cmd(unsigned int command, void * inBuf, void * outBuf)
{
    ...
    case WRITE_WITHOUT_ECC:
    {
        ...
        g_no_ecc = 1;
        spi_nand_page_write((fwd->block * pchip->chip_block_size) + (fwd->page * pchip->chip_page_size), fwd->offset, fwd->data, fwd->amount);
        g_no_ecc = 0;
        break;
    }
    ...
    case GET_FEATURE:
        return(spi_nand_get_cmd(FLASH_GFEAT, *(unsigned char *)inBuf));

    case SET_FEATURE:
        spi_nand_set_feat(*(unsigned char *)inBuf, *(unsigned char *)outBuf);
        break;
