...
#if 1 /* AX6000 */
/*  set HWLED to mode B at loader stage */
...
# if 1 /* AX6000 */
	/* set HWLED to mode B for port 7, */
	ledctrl = &ETHSW_REG->led_ctrl[6].led_encoding;
	bcm_ethsw_set_led_reg(ledctrl);
	ETHSW_REG->led_ctrl[6].led_ctrl = 0x3;
	ETHSW_REG->led_ctrl[6].led_encoding_sel = 0x290;
	ETHSW_REG->led_ctrl[6].led_encoding = 0xffffffff;	
#endif
