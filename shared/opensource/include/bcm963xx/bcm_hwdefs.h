...
#define BOARD_ID                "U12H376"	/* Foxconn added */
/*****************************************************************************/
...
/* AX6000 10 extra MTD */
#define NAND_EXTRA_PARTITION_10

#ifdef NAND_EXTRA_PARTITION_10
#define BCM_MAX_EXTRA_PARTITIONS 10
#else
...
#define UP_BOARD_ID_LEN 32 /* foxconn added */
...
typedef struct
{
	...
	*/
	...
	unsigned short size;
#ifdef NAND_EXTRA_PARTITION_10
    unsigned char ucUnused4[2]; /* data struct 4 bytes alignment */
#endif
...
# define NVFEAT_RESERVED    (1<<31)     // bit reserved for erasure detection
    ...
    char szFirmwareUpgradeBoardId[UP_BOARD_ID_LEN]; /* foxconn added Bob, 07/12/2010, for TFTP firmware upgrade */
    char szRegionNum[2]; //bd_util.h
#ifdef NAND_EXTRA_PARTITION_10
    char chUnused[202]; /* 268 - (10*4 - 4*2) -32 -2 = 202 */
#else
